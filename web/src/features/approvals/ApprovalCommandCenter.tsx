...
import type { DailyPlanSummary, WorkOrderListItem } from "../../api/types";
...
export type ApprovalSourceKey = "workOrders" | "dailyPlans";

interface ApprovalCommandCenterProps {
  workOrders: WorkOrderListItem[];
  dailyPlans: DailyPlanSummary[];
  failures: ApprovalSourceKey[];
}
  ...
  const sourceCards = [
    {
      ...
      Icon: CheckSquare,
      failed: failures.includes("workOrders"),
    },
    {
      key: "daily-plans",
      ...
      Icon: CalendarCheck,
      failed: failures.includes("dailyPlans"),
    },
    {
      key: "target-change",
  ...
  return (
    <Card className="grid gap-5 bg-ink text-white" aria-labelledby="approval-command-center-title">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        {sourceCards.map(({ key, title, description, count, href, action, Icon, failed }) => (
          <div
            key={key}
            className={cn(
              "grid gap-3 rounded-lg border border-slate-700 bg-slate-950/50 p-4",
              failed && "border-red-300 bg-red-950/30",
            )}
          >
