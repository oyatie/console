...
export const financeModuleScreen: ModuleScreenConfig = {
  ...
  data: {
    list: "/api/v1/finance-gl/vouchers",
    detail: "/api/v1/finance-gl/vouchers/{voucher_id}",
    create: "/api/v1/finance-gl/vouchers",
    post: "/api/v1/finance-gl/vouchers/{voucher_id}/post",
    reverse: "/api/v1/finance-gl/vouchers/{voucher_id}/reverse",
  ...
  // S14 re-model: real counts over the fetched page (finance-gl VoucherSummary
  // FSM); 당월수입/당월지출 are deliberately not modeled — the backend carries no
  // revenue/expense account classification to drill into (§4-25-⑥, no
  // fabricated data).
  statbar: [
    {
      key: "pending",
      labelKey: "console.modules.finance.stats.pending",
      tone: "warn",
      source: "finance_voucher.status not in POSTED|REVERSED",
      requiresBackend: true,
    },
    {
      key: "postedThisMonth",
      labelKey: "console.modules.finance.stats.postedThisMonth",
      tone: "ok",
      source: "finance_voucher.status=POSTED posted_at in current_period",
      requiresBackend: true,
    },
    {
      key: "postedAmountThisMonth",
      labelKey: "console.modules.finance.stats.postedAmountThisMonth",
      tone: "info",
      source: "sum(finance_voucher.debit_total_won) status=POSTED posted_at in current_period",
      requiresBackend: true,
    },
    {
      key: "auto",
      labelKey: "console.modules.finance.stats.auto",
      tone: "accent",
      source: "finance_voucher.source_object_type is not null",
      requiresBackend: true,
    },
  ],
  search: {
  ...
  detail: {
    fields: [
      ...
      { key: "postedAt", labelKey: "console.modules.finance.detail.postedAt", variant: "mono" },
      { key: "documentFlow" },
      { key: "balanceCheck" },
      { key: "totalDebitWon" },
      { key: "totalCreditWon" },
      { key: "glAccountSummary", variant: "mono" },
      { key: "branchScope" },
      { key: "createdBy" },
      { key: "approvedBy" },
    ],
