...
- **AD-2 Workspace state**: **Zustand** for the window/panel/workspace engine only. Data fetching stays on the existing `openapi-fetch` wrapper + read cache. No React-Query migration.
  - **Accepted deviation (comms store, UI-M2b)**: the comms Zustand store (`web/src/features/comms/store.ts`) holds fetched notification/mail data and unread counts, not just workspace UI state. Rationale: the rail needs one piece of cross-shell shared state that the sidebar badges, the topbar bell, and the rail all read, and that the realtime WS hub merges live socket events into (`applyNotificationCreated`, messenger fan-out) — behaviour the openapi-fetch read-cache alone does not provide (it is per-instance and has no push channel). Fetching still goes through the shared typed client (issue #219 removed the last hand-rolled `fetch`), so the read-cache, single-flight 401 refresh, and `X-Auth-Transport` cookie transport are preserved; the store owns only the merged, realtime-updated projection.
