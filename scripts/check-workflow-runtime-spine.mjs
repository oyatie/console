...
function requireSqlIncludes(sql, needle, label) {
  assert(sql.includes(needle.toLowerCase()), label, `${migrationPath}: must include SQL fragment ${JSON.stringify(needle)} (${label})`);
}
...
for (const table of tables) {
  ...
  requireSqlIncludes(sql, `grant select`, `${table}: grants present`);
}
