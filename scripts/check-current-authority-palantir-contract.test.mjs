...
const ARTIFACT_BODIES = Object.freeze({
  ...
  [BODY_GATE_REL]: `const RESOLVED_FLOOR = 106;
...
function landedRoadmap() {
  return `# Roadmap
The current body-gate floor is 106 resolved of 291 census; BODY_UNDECIDABLE_MAX is 175.
`;
}
...
describe("check-current-authority-palantir-contract", () => {
  ...
  it("fails when ROADMAP still claims the stale 53/291 body-gate floor", () => {
    ...
    assert.ok(
      result.findings.some((finding) => /106 resolved of 291/.test(finding.message)),
      JSON.stringify(result.findings, null, 2),
    );
  });
