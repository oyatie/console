...
// setup-rust is now the ONLY thing in CI that names a Rust version, so its body
// carries the assurance that twelve `toolchain:` inputs used to carry. Locking
// it here is not extra ceremony -- it is where that coverage went when the
// literals left the workflow. What it holds: the pinned dtolnay digest, and the
// fail-closed parse that refuses a rust-toolchain.toml with zero or two
// channels rather than silently taking the first.
const setupRustActionFile = readFileSync(
...
export function evaluateCiPreflight(
  ...
  setupRustAction = setupRustActionFile,
) {
  const failures = [];
  let workflowModel;
  try {
  ...
  try {
    const actionModel = yaml.load(setupRustAction);
    if (sha256(stableSerialize(actionModel)) !== setupRustActionSha256) {
      failures.push("setup-rust must preserve its exact toolchain-resolution contract");
    }
  } catch (error) {
    failures.push(`setup-rust action must parse as YAML: ${error.message}`);
  }
