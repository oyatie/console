...
: "${MNT_IOS_LOCAL_PGPORT:=55444}"
: "${MNT_IOS_LOCAL_BACKEND_PORT:=8137}"
: "${MNT_IOS_LOCAL_CONTAINER:=mnt-iosui}"
: "${DEVELOPER_DIR:=/Applications/Xcode.app/Contents/Developer}"
export DEVELOPER_DIR

DB=mnt_ios_ui
APP_PASSWORD=mnt-e2e-owner-change-me
WORK="$ROOT/.tmp/ios-ui-local"
AUTH="$WORK/auth"
DERIVED="$WORK/derived"
MNT_APP_BIN="$ROOT/.tmp/cargo-target-e2e/debug/mnt-app"
...
SQLX_OFFLINE=true CARGO_TARGET_DIR="$ROOT/.tmp/cargo-target-e2e" \
  cargo build --manifest-path backend/Cargo.toml -p mnt-app
test -x "$MNT_APP_BIN"

step "postgres on 127.0.0.1:$MNT_IOS_LOCAL_PGPORT"
if ! docker exec "$MNT_IOS_LOCAL_CONTAINER" pg_isready -U mntadmin -d postgres >/dev/null 2>&1; then
  docker rm -f "$MNT_IOS_LOCAL_CONTAINER" >/dev/null 2>&1 || true
  # --rm so an interrupted run cannot leave an orphaned volume behind.
  docker run -d --rm --name "$MNT_IOS_LOCAL_CONTAINER" \
    -p "127.0.0.1:$MNT_IOS_LOCAL_PGPORT:5432" \
    -e POSTGRES_DB=postgres -e POSTGRES_USER=mntadmin -e POSTGRES_PASSWORD=iosui-admin \
    "$PG_IMAGE" >/dev/null
  for _ in $(seq 1 60); do
    docker exec "$MNT_IOS_LOCAL_CONTAINER" pg_isready -U mntadmin -d postgres >/dev/null 2>&1 && break
    sleep 1
  done
  docker cp ops/postgres-reconcile-topology.sh "$MNT_IOS_LOCAL_CONTAINER:/reconcile.sh" >/dev/null
  docker exec \
    ...
    -e MNT_RT_POSTGRES_PASSWORD=mnt-e2e-runtime-change-me \
    -e MNT_LEAVE_COMMAND_POSTGRES_PASSWORD=mnt-e2e-leave-command-change-me \
    -e MNT_ONTOLOGY_COMMAND_POSTGRES_PASSWORD=mnt-e2e-ontology-command-change-me \
    -e MNT_PLATFORM_FORCE_COMMAND_POSTGRES_PASSWORD=mnt-e2e-platform-force-command-change-me \
    "$MNT_IOS_LOCAL_CONTAINER" bash /reconcile.sh >/dev/null
fi

pg_env=(
  E2E_PG_SUPERUSER=mntadmin E2E_PG_SUPERUSER_PASSWORD=iosui-admin
  E2E_PG_HOST=127.0.0.1 E2E_PG_PORT="$MNT_IOS_LOCAL_PGPORT" E2E_DB_NAME="$DB"
)

step "database"
env "${pg_env[@]}" MNT_APP_BIN="$MNT_APP_BIN" bash e2e/harness/db.sh >"$WORK/db.log" 2>&1
tail -1 "$WORK/db.log"

step "backend on 127.0.0.1:$MNT_IOS_LOCAL_BACKEND_PORT"
...
import socket,sys
s=socket.socket()
sys.exit(0 if s.connect_ex(('127.0.0.1', $MNT_IOS_LOCAL_BACKEND_PORT)) != 0 else 1)
"; then
  cat >&2 <<MSG
port $MNT_IOS_LOCAL_BACKEND_PORT is already serving. Something else owns it —
...
directory. Either stop that process, or run this shard on another port:
  ...
  MNT_IOS_LOCAL_BACKEND_PORT=8138 scripts/run-ios-ui-shard.sh $SHARD
MSG
  exit 1
fi
env "${pg_env[@]}" MNT_APP_BIN="$MNT_APP_BIN" MNT_IOS_COLDSTART_OTP="$(openssl rand -hex 32)" \
  node scripts/boot-ios-ui-backend.mjs "$ROOT" "$AUTH" "$MNT_IOS_LOCAL_BACKEND_PORT" \
  >"$WORK/boot.log" 2>&1
...
xcodebuild -resolvePackageDependencies -onlyUsePackageVersionsFromResolvedFile \
  -project MaintenanceField.xcodeproj -scheme MaintenanceFieldApp >"$WORK/resolve.log" 2>&1
...
if [[ -s "$UUID_FILE" ]] && xcrun simctl list devices -j \
  ...
  UUID="$(xcrun simctl create "mnt-local-$SHARD" "$SIM_TYPE" "$SIM_RUNTIME")"
...
xcodebuild build-for-testing -onlyUsePackageVersionsFromResolvedFile \
  -project MaintenanceField.xcodeproj -scheme MaintenanceFieldApp \
...
PGPASSWORD="$APP_PASSWORD" psql -h 127.0.0.1 -p "$MNT_IOS_LOCAL_PGPORT" -U mnt_app -d "$DB" \
...
export MNT_UITEST_BASE_URL="http://127.0.0.1:$MNT_IOS_LOCAL_BACKEND_PORT"
python3 -c 'import json,sys;print(json.dumps({"otp":sys.argv[1]}))' "$OTP" > "$AUTH/otp.json"
curl --fail --silent --show-error -X POST "$MNT_UITEST_BASE_URL/api/v1/auth/otp/redeem" \
  -H 'content-type: application/json' -H 'x-maintenance-client: mobile' \
  --data-binary @- < "$AUTH/otp.json" > "$AUTH/tokens.json"
rm -f "$AUTH/otp.json"
MNT_UITEST_ACCESS_TOKEN="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["access_token"])' "$AUTH/tokens.json")"
MNT_UITEST_REFRESH_TOKEN="$(python3 -c 'import json,sys;print(json.load(open(sys.argv[1]))["refresh_token"])' "$AUTH/tokens.json")"
export MNT_UITEST_ACCESS_TOKEN MNT_UITEST_REFRESH_TOKEN
export MNT_UITEST_WORK_ORDER_ID_DETAIL=00000000-0000-0000-0000-000000f00004
export MNT_UITEST_WORK_ORDER_ID_START=00000000-0000-0000-0000-000000f00003
export MNT_UITEST_WORK_ORDER_ID_REPORT=00000000-0000-0000-0000-000000f00004
export MNT_UITEST_WORK_ORDER_ID_REPORT_SUCCESS=00000000-0000-0000-0000-000000f00005
export MNT_UITEST_WORK_ORDER_ID_ADMIN_APPROVE=00000000-0000-0000-0000-000000f00007
export MNT_UITEST_WORK_ORDER_ID_ADMIN_REJECT=00000000-0000-0000-0000-000000f00008
export MNT_UITEST_WORK_ORDER_ID_CAMERA=00000000-0000-0000-0000-000000f00004
export MNT_UITEST_MESSENGER_THREAD_ID=00000000-0000-0000-0000-000000c10001
export MNT_UITEST_MESSENGER_INITIAL_MESSAGE_ID=00000000-0000-0000-0000-000000c20001
...
( cd "$ROOT" && python3 scripts/patch-ios-xctestrun.py "$XCTESTRUN" \
  ...
  --env MNT_UITEST_BASE_URL --env MNT_UITEST_ACCESS_TOKEN --env MNT_UITEST_REFRESH_TOKEN \
  --env MNT_UITEST_WORK_ORDER_ID_DETAIL --env MNT_UITEST_WORK_ORDER_ID_START \
  --env MNT_UITEST_WORK_ORDER_ID_REPORT --env MNT_UITEST_WORK_ORDER_ID_REPORT_SUCCESS \
  --env MNT_UITEST_WORK_ORDER_ID_ADMIN_APPROVE --env MNT_UITEST_WORK_ORDER_ID_ADMIN_REJECT \
  --env MNT_UITEST_WORK_ORDER_ID_CAMERA --env MNT_UITEST_MESSENGER_THREAD_ID \
  --env MNT_UITEST_MESSENGER_INITIAL_MESSAGE_ID )
