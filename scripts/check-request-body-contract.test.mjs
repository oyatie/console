...
describe("request body contract gate", () => {
  ...
  it("resolves every anchor and stays above the floor against this repository", () => {
    ...
    assert.ok(resolved >= 80, `resolver degraded: expected at least 80 resolved operations, got ${resolved}`);
  });
  ...
  // slot and reads as coverage. The floor of 80 resolved operations means only the real
...
`;
  ...
  it("enforces the enum floor independently of body resolution", () => {
    ...
    assert.equal(result.status, 1, `${result.stdout}${result.stderr}`);
    assert.match(result.stderr, /enum-resolved 0.*below the floor of 17/);
  });
});
...
describe("live request body census", () => {
  it("binds the exact source-first body and enum populations to the reviewed register", () => {
    ...
    assert.equal(report.skipped, 201);
    assert.equal(report.enumCandidates, 39);
    assert.equal(report.enumResolved, 17);
