...
describe("request body contract gate", () => {
  ...
  it("resolves every anchor and stays above the floor against this repository", () => {
    ...
    assert.ok(resolved >= 98, `resolver degraded: expected at least 98 resolved operations, got ${resolved}`);
  });
  ...
  // slot and reads as coverage. The floor of 98 resolved operations means only the real
...
describe("live request body census", () => {
  it("binds the exact source-first body and enum populations to the reviewed register", () => {
    ...
    assert.equal(report.resolved, 108);
    assert.equal(report.skipped, 183);
    assert.equal(report.enumCandidates, 45);
    assert.equal(report.enumResolved, 19);
    assert.equal(report.enumSkipped, 26);
    assert.deepEqual(report.findings, []);
