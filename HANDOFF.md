...
| Path | Role |
|------|------|
| `backend/` | Rust Cargo workspace (modular monolith, `console-` prefix, layering `domain ← application ← adapter ← {rest,worker} ← app`) |
| `backend/crates/kernel/core` | `console-kernel-core` — typed IDs, AuditEvent, BranchScope, TraceContext, errors, Clock |
...
| `android/` | Kotlin/Compose field app Gradle project (`console-android`) with JVM unit/Compose/Roborazzi and managed-device instrumented test topology. Play/internal-track release still requires signing and service-account secrets. |
...
| `ios/` | SwiftPM/SwiftUI field app (`Console`) with core/app/tests/UITests. `ios/project.yml` generates a CI-only Xcode project for Simulator XCUITest; do not infer a committed distributable Xcode project/workspace from it. |
| `.github/workflows/ios-ui-tests.yml` | Hermetic XCUITest + strict accessibility-audit gate on one job-local GitHub-hosted `macos-26` VM, including untrusted/public pull requests. It rejects any reusable self-hosted runner boundary; a future self-hosted lane requires separately governed ephemeral/JIT runners with teardown attestation. The job verifies Xcode 26.6 build `17F113` with Apple Swift 6.3.3 in strict Swift 6 language mode and the iOS 26.5 Simulator runtime, verifies the exact candidate SHA, builds checksum-pinned PostgreSQL 18.4 and that SHA's Rust backend, and keeps Cargo/Rust, PostgreSQL, backend, DerivedData, and result paths under one owned runner-temporary root. Each test-class shard redeems a fresh random one-use OTP for a new session and uses a measured class-specific 45-to-600-second hard bound under the 15-minute access-token TTL; production token rotation exists, but this gate does not depend on refresh to remain valid. Structured results must equal the source-discovered XCTest set across all shard `.xcresult` bundles with zero skips/failures/errors. Before upload, CI scans the results for every raw OTP/access/refresh value. It uploads results first, then unconditionally performs identity-aware backend shutdown, PostgreSQL stop proof, Simulator deletion proof, and owned-root deletion. No external backend/session secret or fork skip path remains; CI-only loopback ATS does not change the production plist or release networking policy. |
