...
For the local host development loop, `npm run dev:up` prepares private local
TLS and retains the selected database identity before migration. It then runs
migration, Account custody finalization, and application startup in that order.
Docker must share the checkout and its private files with the container VM.

For an independently provisioned container stack, supply the operator transport
before parsing or starting Compose. These files are not application credentials:
...
export ACCOUNT_CUSTODY_TARGET_ENV_FILE=/protected/console/target.env
export ACCOUNT_CUSTODY_PASSWORD_FILE=/protected/console/operator-password
export ACCOUNT_CUSTODY_CA_FILE=/protected/console/ca.crt
export ACCOUNT_CUSTODY_PG_TLS_DIR=/protected/console/server-tls
# Match ownership of the private password file for the nonprivileged finalizer.
export ACCOUNT_CUSTODY_RUN_UID="$(id -u)"
export ACCOUNT_CUSTODY_RUN_GID="$(id -g)"
...
`POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_ADMIN_USER`, and the
five `ACCOUNT_CUSTODY_EXPECTED_*` values consumed by
`postgres-finalize-account-custody.sh`: operator, system identifier, database,
database OID, and TLS host. Retain them from independent provisioning, never
derive them from the finalizer's target connection. Missing or mismatched inputs
stop startup. The password file must match the separately provisioned database
operator and be readable only by the selected finalizer user.

Local certificates are retained for 30 days; an expired certificate, changed
database identity or changed operator password stops the local launcher instead
of silently replacing trust. Rotate local credentials explicitly, with PostgreSQL
stopped, and retain old inventory for diagnosis. No production trust material is
generated by the local helper.

Argo runs operator network policies at PreSync wave `-30`, migration at `-20`,
and Account finalization at `-10`. The namespace, database, DNS, protected
`console-account-custody-target` ConfigMap and operator/CA Secrets must already
exist. The operator image remains `local-unpublished` until independently built,
reviewed and signed for deployment; these declarations do not certify cluster
provisioning, CNI enforcement, HA or production exposure.
