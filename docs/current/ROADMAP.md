...
3. **Delivery substrate convergence**
   ...
   - The canonical integration and source-release branch is `dev` (not `main`). Ordered mirrors: `dev` → `staging` → `canary` → `production`. Required checks (`authenticate-console-authority`, `Required / CI`, `Required / Security`) must have an executable protected workflow producer on `dev`, and on every branch where they are enforced, before they are required there. Release-please, merge-queue branch filters, CAS cache “main only” seed jobs, and bootstrap `assertLivePullRequestSnapshot` base-ref assumptions are serialized follow-on in this PR or the next so CI is not still keyed only to `main` after the GitHub default-branch switch. Fast-forwarding those names is not live exposure. Environment-branch promotion (live DNS/TLS/exposure) remains HOLD until a later go-live PRODUCT.
