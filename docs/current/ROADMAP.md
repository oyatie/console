...
7. **Leptos acceptance surface**
   ...
   - GET `/_ui` is mounted (#952). Contracts-backed authorized reads (#959): empty shell without `PayrollRunRead`; `data-run-*` from `PayrollRunSummary` required fields with it. Remaining: islands/WASM hydration, persona E2E.
...
- Leptos **shipping** remains **HOLD** until persona-based real-backend E2E evidence. GET `/_ui` being mounted, and #959 authorized reads, do not authorize shipping screens or live exposure.
