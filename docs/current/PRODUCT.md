...
5. A Leptos SSR frontend. ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041); first full-depth vertical is payroll execution. Owner decision (2026-08-28) **starts** the remaining frontend implementation in parallel: a mounted contracts-backed SSR shell with island hydration and selective WASM. Shipping screens, production exposure, and persona E2E remain **HOLD** until the remaining frontend conditions in Holds are satisfied.
...
- Frontend **shipping** remains **HOLD**. ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041); those facts do not authorize shipping screens or live exposure. Owner decision (2026-08-28) **starts** the mounted contracts-backed SSR shell (Leptos 0.9.0-beta from crates.io, SSR, islands, selective WASM hydration; 0.8.x is the rollback if beta blocks production). That start does not put business authority in the client. Remaining ship condition: persona-based real-backend E2E evidence (ADR-0025 §4). Deny-by-omission is server composition. Import/export is not the data-entry base except 자료실 (later, not this slice). The comms rail is not in this slice. Absence of a mounted shell stays green until the change that mounts one; that change is its own PR.
