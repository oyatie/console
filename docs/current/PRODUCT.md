...
5. A Leptos SSR frontend. ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041); first full-depth vertical is payroll execution. A mounted shell remains **HOLD** until the remaining frontend conditions in Holds are satisfied.
...
- Frontend work remains **HOLD**. ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041); those facts do not clear this HOLD, do not resume frontend implementation, and do not authorize shipping screens. Remaining conditions: a mounted contracts-backed SSR shell and persona-based real-backend E2E evidence (ADR-0025 §4). ROADMAP's Leptos/frontend HOLD remains until those conditions are actually satisfied. First full-depth vertical is payroll execution. Stack: Leptos 0.9.0-beta (crates.io) with SSR, islands, and selective WASM hydration; 0.8.x is the rollback if beta blocks production. The client has no business authority; deny-by-omission is server composition. Import/export is not the data-entry base except 자료실 (later, not this slice). The comms rail is not in this slice. Absence of a mounted shell stays green until the change that mounts one.
