...
5. A Leptos SSR frontend. ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041); first full-depth vertical is payroll execution. GET `/_ui` serves an empty deny-by-omission SSR shell (`console-payroll-ui` nested by `console-app`; #952). Remaining frontend work: islands/WASM hydration, contracts-backed authorized reads, persona E2E. Shipping screens, production exposure, and persona E2E remain **HOLD**.
