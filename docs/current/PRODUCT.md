...
The overlay already exists (migration 0060 `groups` / `group_memberships`, grants `GROUP_ADMIN` / `GROUP_VIEWER` / `GROUP_FINANCE`, fail-closed `group_member_org_ids`, `console-platform-group::consolidated_read`). Minting a Group row for every Company that lacks one, and requiring `organizations.group_id`, is admitted work. It does not add `ObjectKey::Group` in this record. The canonical `ObjectKey` roster stays six until a later reviewed design proves Group identity writes need a port.
...
- Frontend **shipping** is admitted (owner 2026-08-29). ADR-0030 substrate gates are green and `Layer::Ui` is accepted (ADR-0041). GET `/_ui` is mounted (deny-by-omission SSR, Leptos 0.9.0-beta, #952). Contracts-backed authorized reads (#959), the `AuthorizedRuns` island (#962), and WASM hydration of that island (#964) do not put business authority in the client and are not persona E2E. Persona-based real-backend E2E (ADR-0025 §4) remains the bar for calling a screen shipped. React tombstone paths stay absent. Deny-by-omission is server composition. Import/export is not the data-entry base except 자료실 (later, not this slice). The comms rail is not in this slice. 0.8.x is the rollback if 0.9.0-beta blocks production. Live exposure remains **HOLD**.
...
- Cloning Intelligence into Console is **HOLD** until Intelligence names a SHA-bound stable base. Until that clone lane, AI judgment remains out of Console's tenant product. Console-owned seams are admitted; they are not a clone and not an Intelligence HTTP surface on Console.
