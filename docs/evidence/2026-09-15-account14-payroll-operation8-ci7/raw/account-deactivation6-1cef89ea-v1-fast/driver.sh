...
export FENCE_LABEL="account-deactivation6-$(openssl rand -hex 16)"
...
entries=[e for e in data['entries'] if e.get('name')=='identity-adapter-postgres-deactivate-revokes-credentials-pg']
assert len(entries)==1
e=entries[0]
assert e['package']=='console-identity-adapter-postgres' and e['test']=='deactivate_revokes_credentials'
expected=['cargo','test','--locked','--manifest-path','backend/Cargo.toml','-p','console-identity-adapter-postgres','--test','deactivate_revokes_credentials','--','--test-threads=1']
assert e['cargo_argv']==expected, 'unreviewed underlying test command'
# Preserve original unfiltered identity owner target, including its legacy test.
...
cargo() {
  ...
  if [[ $# == 7 && "$1" == test && "$2" == --locked && "$3" == --manifest-path && "$4" == "${FENCE_ROOT}/backend/Cargo.toml" && "$5" == --no-run && "$6" == -p && "$7" == console-identity-adapter-postgres ]]; then
    printf '%s\n' 'matched exact seven-argument prebuild; appended --test deactivate_revokes_credentials' >> "${FENCE_BUILD_RECORD}"
    command cargo "$@" --test deactivate_revokes_credentials
  else
    command cargo "$@"
  fi
}
...
export -f cargo docker docker_bounded
...
cd "${FENCE_ROOT}"
set +e
bash tools/ci/cargo_needs_postgres.sh --map "${map_path}" --only identity-adapter-postgres-deactivate-revokes-credentials-pg --num-threads 1 > "${evidence_dir}/probe.log" 2>&1
probe_status=$?
set -e
...
data={'driver_sha256':hashlib.sha256(driver.read_bytes()).hexdigest(),
 ...
 'storage':'owned labeled PostgreSQL container using original harness storage; no durability proof',
