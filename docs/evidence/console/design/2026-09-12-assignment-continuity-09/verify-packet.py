...
core = ['entry.txt', 'assignment-contract.txt', 'acceptance-plan.csv',
...
assert [r['id'] for r in cases] == [f'A{i:02}' for i in range(1, len(cases) + 1)]
assert [r['id'] for r in screens] == [f'U{i:02}' for i in range(1, len(screens) + 1)]
sections = set(re.findall(r'^WA1\.\d+\b', (p / 'assignment-contract.txt').read_text(), re.M))
