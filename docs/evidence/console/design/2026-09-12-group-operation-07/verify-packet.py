...
core = ['entry.txt', 'group-contract.txt', 'organization-steering.txt',
        'acceptance-plan.csv', 'screen-states.csv', 'source-identities.json', 'scope.json']
for name in core:
...
assert len(screens) == scope['screen_states'] == 14
assert [r['id'] for r in scenarios] == [f'G{i:02}' for i in range(1, 51)]
contract = (p / 'group-contract.txt').read_text()
declared = set(re.findall(r'^GO1\.\d+\b', contract, re.M))
