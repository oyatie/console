...
### Scope/design alignment before the next implementation slice

- Reconcile current scope with the full Foundry capability target and HR/shared workflow focus in PRODUCT; keep a dated leaf capability/acceptance/gap register. Do not claim full parity from a family-level inventory.
- Bind the one-Group/six-Company/2,000-person Console-operated pilot and same-tenant whole-Group/Company-set operations to explicit policy, screen, input, and recovery contracts.
- Preserve payroll draft execution as the first full-depth Leptos vertical. Reuse the existing canonical create/submit/decide port and original payroll writers. Prove the native input -> roster/basis -> calculation -> review path; an import-backed run and a separate contract-wage individual draft are not already one connected native workflow.
- Resolve state/revision/recalculation and natural-person approval parity across the direct REST and canonical action entry paths. Existing replay semantics differ; do not add a second writer or silently change compatibility.
- Verify/fix lane admission's distinction between product assertion failure and environment/timeout/zero-test results, and prove the same probe rejects a known-bad fixture before the next implementation fan-out. Validate effective child instructions and immutable run evidence without blindly removing historical workaround flags.
- Prepare exact design/test candidates and follow DELIVERY's independent review and SHA-bound approvals. These records and scope changes do not admit an implementation lane or clear production/legal/Intelligence HOLDs.

### Existing ordered program
...
7. **Leptos acceptance surface**
   ...
   - Shipping screens are composed (#976). Payroll drill-through hrefs and empty-vs-denied SSR landed (#982). Persona real-backend E2E (ADR-0025 §4) on org/HR/payroll GET `/_ui` landed (#978). Production exposure stays **HOLD**. Import/export is not the data-entry base except 자료실. Shared communications retain their own persistent workspace and are delivery-sequenced after the first payroll increment; they are not excluded from the product target.
