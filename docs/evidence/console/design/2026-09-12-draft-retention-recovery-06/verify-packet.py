...
core = ['entry.txt', 'retention-contract.txt', 'acceptance-plan.csv',
...
assert len(scenarios) == scope['scenarios'] == 39
assert len(screens) == scope['screen_states'] == 13
assert [r['id'] for r in scenarios] == [f'R{i:02}' for i in range(1, 40)]
contract = (p / 'retention-contract.txt').read_text()
declared = set(re.findall(r'^RR1\.\d+\b', contract, re.M))
