...
if admin -d db_a -1 -f /experiment/transfer-omitted.sql > /experiment/transfer-omitted.log 2>&1; then
  echo 'FAIL omitted transfer escaped checker'; exit 1
fi
grep -q 'wrong owner account_terms_release_receipts' /experiment/transfer-omitted.log
admin -d db_a -f /experiment/check-rolled-back.sql
echo 'PASS omitted sixth transfer detected and transaction rolled back'
admin -d db_a -1 -f /experiment/transfer.sql
admin -d db_a -f /experiment/check-custody.sql
if admin -d db_a -1 -f /experiment/extra-column-grant.sql > /experiment/extra-column-grant.log 2>&1; then
  echo 'FAIL extra column grant escaped checker'; exit 1
fi
grep -q 'custody privilege console_rt accounts' /experiment/extra-column-grant.log
admin -d db_a -f /experiment/check-custody.sql
echo 'PASS extra direct column grant detected and rolled back'
...
for table in accounts account_security account_security_events account_terms_acceptances account_terms_head account_terms_release_receipts; do
 PGPASSWORD="$CONSOLE_RT_POSTGRES_PASSWORD" psql -X -h127.0.0.1 -U console_rt -d db_a -v ON_ERROR_STOP=1 -q -At -F '|' -c 'SELECT current_user,current_database(),1' > /experiment/login-positive-$table.log 2>&1
 test "$(cat /experiment/login-positive-$table.log)" = 'console_rt|db_a|1'
 if PGPASSWORD="$CONSOLE_RT_POSTGRES_PASSWORD" psql -X -h127.0.0.1 -U console_rt -d db_a -v ON_ERROR_STOP=1 -v VERBOSITY=verbose -q -c "SELECT * FROM public.$table LIMIT 0" > /experiment/refusal-$table.log 2>&1; then echo 'FAIL runtime read'; exit 1; fi
 grep -Eq "^ERROR:  42501: permission denied for table $table$" /experiment/refusal-$table.log
done
