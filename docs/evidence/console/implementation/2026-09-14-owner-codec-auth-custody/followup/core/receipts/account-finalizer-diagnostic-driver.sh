...
export FENCE_LABEL="account-finalizer-diagnostic-$(openssl rand -hex 16)"
...
e['cargo_argv'].insert(e['cargo_argv'].index('--'),'account_custody_lifecycle::concurrent_finalizers_have_complete_request_outcomes_and_one_final_state')
...
docker() {
  ...
  if [[ "${1:-}" == run && "${name}" == console-cargo-postgres-* && "${matched_image}" == 1 ]]; then
    ...
    command docker run --tmpfs /var/lib/postgresql:rw,size=1g \
...
data={'driver_sha256':hashlib.sha256(driver.read_bytes()).hexdigest(),
 ...
 'storage':'owned labeled PostgreSQL container only; 1GiB tmpfs; no durability proof',
