...
name='console-fk-owner-'+uuid.uuid4().hex
...
try:
 ...
 for title,grant in [('owner SELECT only',None),('owner REFERENCES too','GRANT REFERENCES ON public.accounts TO fixture_owner;'),('owner UPDATE(id) instead of REFERENCES','REVOKE REFERENCES ON public.accounts FROM fixture_owner; GRANT UPDATE(id) ON public.accounts TO fixture_owner;')]:
  if grant:
 ...
 pathlib.Path('/private/tmp/fk-owner-permissions-probe-result.json').write_text(json.dumps(record,indent=2)+'\n')
