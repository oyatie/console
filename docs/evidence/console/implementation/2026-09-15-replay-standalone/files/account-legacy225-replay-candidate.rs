...
async fn seed_legacy(pool: &PgPool) -> LegacyFixture {
    ...
    let restored: Value = sqlx::query_scalar(r#"WITH fixture_subjects AS (
...
SELECT jsonb_build_object(
    ...
    'unused_fixture_edges', jsonb_build_object(
        ...
        .bind(&subjects).bind(&audit_ids).fetch_one(&mut *tx).await.unwrap();
    assert!(
        restored["tables"] == *tables,
        "complete historical rows differ; no sensitive dump"
    );
    assert!(restored["migration_version"] == 225);
    assert!(restored["migration_checksums"] == saved["capture"]["migration_checksums"]);
