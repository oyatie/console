...
| Lint | `cargo clippy --all-targets -D warnings -p console-notifications-{domain,application,adapter-postgres,rest} -p console-platform-realtime -p console-platform-auth` | GREEN (exit 0) |
| Domain tests | `cargo test -p console-notifications-domain` | 5/5 GREEN |
| Adapter tests | `cargo test -p console-notifications-adapter-postgres` vs dev Postgres 55432 (sqlx::test scratch DBs via `console_cluster_admin`; store runs on a genuine `SET ROLE console_rt` pool) | 8/8 GREEN in 4.03s |
| REST tests | `cargo test -p console-notifications-rest` (real router, ES256 JWT, console_rt pool via `console_platform_test_support::runtime_role_pool`) | 2/2 GREEN |
| App E2E | `cargo check -p console-app --tests` | STILL BLOCKED: `console-production-rest` 4 errors (`E0063 plan_digest`, E0308, E0382, lifetime) + `console-facilities-rest` 3 errors (`E0106`, `E0277 DueCaseBody: Serialize`, `E0599 Hmac::new_from_slice`) — other lanes' mid-flight crates, untouched per hot-check discipline |
...
`console_cluster_admin` dev URL — `console_app@55432` lacks CREATEDB, so
...
- **FORCE RLS + org policy on the new table**: `0196_notification_policies_and_object_agg.sql`
  ...
  SELECT/INSERT/UPDATE/DELETE to `console_rt` (DELETE is required — unmute is a real
...
- **Tests genuinely run as console_rt**: both the adapter suite and the REST suite build
  a second pool with `after_connect → SET ROLE console_rt` and hand THAT pool to the
...
- **Design-contract fidelity**: the five chartered routes in
  `design-contract.md` §"console-notifications-rest" match the implementation and the
...
- `console-logistics-adapter-postgres` still fails `clippy -D warnings` on two
...
1. Run `backend/app/tests/notif_routing_api.rs` once `console-production-rest` and
   `console-facilities-rest` compile (their lanes' fixes) — it is committed, rustfmt-clean,
   and cannot be compile-checked until `console-app` builds.
