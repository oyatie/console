...
- `GET|POST /api/v1/hr/attendance-records/me` — self-service attendance transitions. FSM `next_employee_attendance_state`: OFF_DUTY→CLOCK_IN→CLOCKED_IN; CLOCKED_IN→{OUT_FOR_WORK, BUSINESS_TRIP, CLOCK_OUT}; OUT_FOR_WORK/BUSINESS_TRIP→{RETURNED→CLOCKED_IN, CLOCK_OUT}. Idempotency-keyed, append-only, writes `payroll_attendance_material_refs`, **freeze-window gate** via `console_platform_db::period_lock::assert_period_open` (whole txn 409s inside an active lock).
