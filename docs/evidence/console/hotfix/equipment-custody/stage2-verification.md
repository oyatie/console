...
from a `console_buck_admin` cluster admin, `DATABASE_URL` carrying
`options[mnt.sqlx_test_bootstrap]=buck-sqlx-superuser-v1`. Every HTTP assertion
crosses the assembled router on a `SET ROLE console_rt` pool
(`runtime_role_pool`). The shared `console-dev` stack was never touched and
...
$ cargo test -p console-app --test equipment_3r_api
...
|---|---|
| FORCE RLS org isolation on new tables | no new table; `docs_equipment_handover_custody` (0184) is `ENABLE` + `FORCE ROW LEVEL SECURITY` with an `app.current_org` policy and `REVOKE UPDATE, DELETE … FROM console_rt`. `with_audits` arms `app.current_org` before the closure runs (`platform/db/src/audit_tx.rs:111-121`). |
| tested as `console_rt`, not as superuser | every HTTP assertion goes through `runtime_role_pool` (`SET ROLE console_rt` in `after_connect`). Superuser `pool` is used only to seed and to read back assertions. |
...
| audit on every mutation | the custody insert and the FSM transition share one `with_audits` transaction that emits `equipment_3r.case.handover`; the lifecycle story still asserts exactly 8 case audits. `console-gate-audit-coverage` PASSED. |
...
- `8cd1092f` really does add `console-docs-adapter-postgres` + `console-docs-application`
...
```
cargo test -p console-app --test equipment_3r_api                            -> 5 passed; 0 failed
...
SQLX_OFFLINE=true cargo clippy \
  -p console-equipment-{domain,application,adapter-postgres,rest} \
  --all-targets -- -D warnings                                           -> clean
SQLX_OFFLINE=true cargo clippy -p console-app --test equipment_3r_api \
...
cargo run -p console-gate-migration-safety   -> FAILED, PRE-EXISTING
```

`SQLX_OFFLINE=true` is required for clippy (CI sets it,
`.github/workflows/ci.yml:359`): with a live `DATABASE_URL` pointing at the
disposable harness, `console-platform-db`'s compile-time `sqlx::query!` macro tries
...
4. **`console-gate-migration-safety` red on the spine** (§6) — integrator /
...
5. **A third spine-level red, found by widening the clippy scope.** CI runs
   ...
   **zero diff between `4cabe239` and this branch**, so the verdict is
   ...
   own (`cargo clippy -p console-equipment-{domain,application,adapter-postgres,rest}
