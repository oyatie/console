...
async fn publication_release_preserves_exact_source_and_is_nonpayable(pool:PgPool)->TestResult {
    ...
    assert_eq!(publications_before["publications"].as_array().unwrap().len(),1);
    ...
    assert_eq!(s.publication_snapshot(&[a.command_id]).await?,publications_before);
...
async fn publication_replace_preserves_history_and_rejects_stale_slot(pool:PgPool)->TestResult {
    ...
    let (published,release_attempt,approved)=s.publication().await?;let old=published.publications[0].clone();
    ...
    let publications_before=s.publication_snapshot(&[release_attempt.command_id,a.command_id]).await?;
    assert_eq!(publications_before["publications"].as_array().unwrap().len(),2);
    assert_eq!(publications_before["slots"].as_array().unwrap().len(),1);
    ...
    assert_eq!(f.snapshot(approved.created.run_id).await?,before);
    assert_eq!(s.publication_snapshot(&[release_attempt.command_id,a.command_id]).await?,publications_before);
    let replay=completed(payroll::payroll_review_replace_projection(&f.pool,&f.submitter,&a).await?)?;
    assert_eq!(value(&replay)?,value(&actual)?);
    assert_eq!(s.publication_snapshot(&[release_attempt.command_id,a.command_id]).await?,publications_before);Ok(())
}
...
async fn group_activate_requires_all_prepared_original_attempts(pool:PgPool)->TestResult {
    ...
    assert_eq!(stored_attempts.len(),stored.slots.len(),"duplicate actual slot attempt must not collapse in set");
...
async fn group_resume_preserves_manifest_and_original_pending_commands(pool:PgPool)->TestResult {
    ...
    let first_receipt:serde_json::Value=sqlx::query_scalar("SELECT to_jsonb(r) FROM ont_action_command_receipts r WHERE org_id=$1 AND command_id=$2")
        .bind(attempts[0].org_id).bind(attempts[0].command_id).fetch_one(&s.deployment.readback).await?;
    let second=&s.deployment.companies[1];
    let pending=workflow::read_group_child_by_attempt(&second.pool,&second.submitter,&attempts[1]).await?;
    assert_eq!(value(&pending)?["state"],"PREPARED");assert!(pending.receipt.is_none());
    ...
    console_platform_auth::account_session::logout_account_session(&s.deployment.runtime.auth,&s.deployment.verifier,
        &s.deployment.accounts.submitter.account_access_token).await?;
    let blocked=workflow::claim_group_child(&second.pool,&second.worker,&pending.reference).await;
    assert!(matches!(blocked,Err(workflow::GroupDispatchError::OriginalSessionEnded)));
    ...
    let readiness=group::read_group_dispatch_readiness(&s.deployment.pool,&fresh_auth,&active.operation).await?;
    assert_eq!(value(&readiness)?["reason"],"ORIGINAL_SESSION_ENDED");
    let identity=s.group_identity::<group::GroupResume>(&fresh_auth).await?;
    let input=group::GroupResume{operation:active.operation.clone(),confirm_original_intent:true};
    let actual=completed(group::group_operation_resume(&s.deployment.pool,&fresh_auth,group::GroupRegisteredActionRequest{identity:identity.clone(),input:input.clone()}).await?)?;
    ...
    assert_eq!(stored.original_account_id,s.deployment.accounts.submitter.account_id);
    assert_eq!(stored.slots.len(),attempts.len());
    let commands=stored.slots.iter().map(|x|(x.org_id,x.attempt.command_id)).collect::<std::collections::BTreeSet<_>>();
    assert_eq!(commands.len(),stored.slots.len());
    assert_eq!(commands,attempts.iter().map(|x|(x.org_id,x.command_id)).collect());
    let first=stored.slots.iter().find(|x|x.org_id==attempts[0].org_id).ok_or("completed slot missing")?;
    assert_eq!(value(first)?["state"],"SUCCEEDED");assert_eq!(first.receipt.as_ref(),Some(&completed_child.receipt));
    let second=stored.slots.iter().find(|x|x.org_id==attempts[1].org_id).ok_or("pending slot missing")?;
    assert_eq!(value(second)?["state"],"PENDING");assert!(second.receipt.is_none());
    let replay=completed(group::group_operation_resume(&s.deployment.pool,&fresh_auth,group::GroupRegisteredActionRequest{identity:identity.clone(),input}).await?)?;
    assert_eq!(value(&replay)?,value(&actual)?);
    assert_eq!(s.deployment.companies[0].snapshot(inputs[0].0).await?,first_snapshot);
    assert_eq!(s.deployment.companies[1].snapshot(inputs[1].0).await?,second_snapshot);
    let after:serde_json::Value=sqlx::query_scalar("SELECT to_jsonb(r) FROM ont_action_command_receipts r WHERE org_id=$1 AND command_id=$2")
        .bind(attempts[0].org_id).bind(attempts[0].command_id).fetch_one(&s.deployment.readback).await?;
    assert_eq!(after,first_receipt);Ok(())
}
