...
def schema(v,name):
    ...
    errors=list(validator.iter_errors(v));require(not errors,"schema "+name+": "+("; ".join(e.message for e in errors[:2])))
    def scalar_walk(x,s):
        if "$ref" in s:
            k=s["$ref"].split("/")[-1]
...
def owner_frame(command):
    ...
    if g is not None:require(g["request_revision"]=="1","initial gate revision")
    a=command["attempt"]
