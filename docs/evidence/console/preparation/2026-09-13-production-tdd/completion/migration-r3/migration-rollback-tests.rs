...
async fn retained(pool:&PgPool,org:uuid::Uuid)->TestResult<Value> {
    ...
    for table in ["payroll_draft_runs","ont_action_command_receipts","retained_units","retained_payload_locations","content_pins","domain_adoptions","protected_inventory_headers","protected_inventory_members","ont_source_submissions","ont_source_submission_bindings","payroll_input_revisions","payroll_input_units","payroll_calculation_batches","payroll_calculation_units"] {
...
async fn compatible_reader_preserves_new_rows_pending_intent_and_original_replay(pool:PgPool)->TestResult {
    ...
    assert!(matches!(fresh,Err(payroll::PayrollActionError::Owner(console_ontology_adapter_postgres::action30::OwnerActionError::ServingEffectsPaused))));
    assert_eq!(before,retained(&d.readback,f.org).await?);
