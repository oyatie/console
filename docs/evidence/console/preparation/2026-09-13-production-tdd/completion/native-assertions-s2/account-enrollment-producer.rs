...
pub struct AccountBootstrapPhase {
    ...
    pub transfer_recipient:EnrolledAccount,
}
pub async fn enroll_fixture_accounts(auth_pool:&PgPool,service:&PasskeyService,verifier:&JwtVerifier,origin:&Url)->TestResult<AccountBootstrapPhase> {
    ...
    let transfer_recipient=enroll_account(auth_pool,service,verifier,origin,"Native independent transfer recipient").await?;
    Ok(AccountBootstrapPhase{submitter,reviewer,same_human_alias,transfer_recipient})
}
