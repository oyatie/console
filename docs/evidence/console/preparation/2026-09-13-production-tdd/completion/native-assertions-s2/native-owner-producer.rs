...
use crate::binder_sequence::{bind_and_seal, completed, completed_attempt, owner_error, ProducerStop};
...
pub async fn complete_native(
    ...
    let progression = async {
        for _ in 0..64 {
            payroll::advance_native_preparation_once(pool, worker, &original_subject).await.map_err(owner_error)?;
            match payroll::observe_native_command(pool, auth, &original_subject).await.map_err(owner_error)? {
                OwnerExecution::Completed(result) => return completed_native_result(result, attempt),
...
pub async fn produce_native_and_review(
    ...
    let created = completed_attempt(payroll::payroll_create_run(pool, submitter, &create_attempt).await.map_err(owner_error)?, &create_attempt)?;
    ...
    let prepare_control = payroll::read_preparation_control(pool, submitter, created.run_id).await.map_err(owner_error)?;
    ...
    let prepared = complete_native(pool, submitter, worker, &prepare_attempt,
        payroll::payroll_prepare_inputs(pool, submitter, &prepare_attempt).await.map_err(owner_error)?).await?;
    ...
    let close_control = payroll::read_attendance_close_control(pool, submitter, created.run_id).await.map_err(owner_error)?;
    ...
    let closed = completed_attempt(payroll::payroll_close_attendance(pool, submitter, &close_attempt).await.map_err(owner_error)?, &close_attempt)?;
    ...
    let calculate_input = payroll::RunCalculate {
        expected: payroll::read_run_control(pool, submitter, created.run_id).await.map_err(owner_error)?,
    };
    ...
    let calculated = complete_native(pool, submitter, worker, &calculate_attempt,
        payroll::payroll_calculate_run(pool, submitter, &calculate_attempt).await.map_err(owner_error)?).await?;
    ...
    let review = completed_attempt(payroll::payroll_open_review_cycle(pool, submitter, &open_attempt).await.map_err(owner_error)?, &open_attempt)?;
    ...
    let bases = payroll::read_complete_review_bases(pool, submitter, &review.cycle).await.map_err(owner_error)?;
    if bases.is_empty(){return Err(ProducerStop::InvalidActualResult("empty actual review basis set"));}
    ...
    for basis in bases {
        ...
        let submitted = completed_attempt(governance::review_submit(pool, submitter, &submit_attempt).await.map_err(owner_error)?, &submit_attempt)?;
        ...
        let decided = completed_attempt(governance::review_decide(pool, independent_reviewer, &decision_attempt).await.map_err(owner_error)?, &decision_attempt)?;
        if decided.decision_id.is_none() {
            return Err(ProducerStop::InvalidActualResult("decision missing"));
        }
        decisions.push(decided);
    }
    let completion = payroll::read_review_completion(pool, submitter, &review.cycle).await.map_err(owner_error)?;
