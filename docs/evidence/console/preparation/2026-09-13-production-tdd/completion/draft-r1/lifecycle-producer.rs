...
pub async fn publish_upgrade(f: &Fixture, current: &InputSchemaRef) -> TestResult<UpgradeFacts> {
    ...
    let partial = schema::publish_mapping_revision(&f.pool, &f.submitter, UnadmittedControl {
        command_id: Uuid::new_v4(), input: schema::MappingProposal {
            source: current.clone(), target: target.reference.clone(), fields: copies.into_iter().skip(1).collect(),
        },
    }).await?;
