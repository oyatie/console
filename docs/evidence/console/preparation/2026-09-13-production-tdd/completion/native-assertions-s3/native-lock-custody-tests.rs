...
async fn wait_for_waiters(admin:&PgPool,blocker:i32,count:usize)->TestResult<BTreeSet<i32>> {
 let deadline=tokio::time::Instant::now()+std::time::Duration::from_secs(5);
 loop {
  let rows:Vec<i32>=sqlx::query_scalar("SELECT pid FROM pg_stat_activity WHERE datname=current_database() AND $1=ANY(pg_blocking_pids(pid)) AND wait_event_type='Lock'")
   .bind(blocker).fetch_all(admin).await?;
  let pids=rows.into_iter().collect::<BTreeSet<_>>();
  if pids.len()>=count{return Ok(pids);}
  if tokio::time::Instant::now()>=deadline{return Err("actual native lock rendezvous was not observed".into());}
...
async fn native_calculate_two_distinct_commands_rendezvous_on_actual_run_lock(pool:PgPool)->TestResult {
 ...
 let a=payroll::payroll_calculate_run(&f.pool,&f.submitter,&one);
 let b=payroll::payroll_calculate_run(&f.pool,&f.submitter,&two);
 ...
 tokio::select! {
  ...
  witness=wait_for_waiters(&admin,pid,2)=>{assert_eq!(witness?.len(),2);}
 }
...
async fn native_runtime_cannot_update_or_delete_actual_immutable_custody(pool:PgPool)->TestResult {
 ...
 for (table,key) in [("payroll_input_revisions","inputs"),("payroll_input_units","units"),
  ...
  for operation in ["UPDATE","DELETE"] {
   ...
   sqlx::query("SELECT set_config('app.current_org_id',$1,true)").bind(f.org.to_string()).execute(&mut *tx).await?;
