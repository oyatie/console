...
#[cfg(test)]mod receipt_tests {
 use super::helpers::validate_history_delta;use serde_json::{json,Value};
 fn inputs()->(Value,Value,Value){
  let before=json!({"head_revision_id":"v1","receipts":[{"command_id":"c1","receipt_id":"r1","revision_id":"v1"}],"revisions":[{"revision_id":"v1","amount":1200000}],"domain_effects":[]});
  let after=json!({"head_revision_id":"v2","receipts":[{"command_id":"c1","receipt_id":"r1","revision_id":"v1"},{"command_id":"c2","receipt_id":"r2","revision_id":"v2"}],"revisions":[{"revision_id":"v1","amount":1200000},{"revision_id":"v2","amount":1234567}],"domain_effects":[]});
  let observation=json!({"acknowledgements":[{"command_id":"c2","receipt_id":"r2","revision_id":"v2"}],"manual_commands":[]});(before,after,observation)
 }
 #[test]fn ordinary_autosave_and_manual_commands_match_same_receipt_structure(){let(b,a,o)=inputs();assert!(validate_history_delta(&b,&a,&o).is_ok());let manual=json!({"acknowledgements":[],"manual_commands":["c2"]});assert!(validate_history_delta(&b,&a,&manual).is_ok());}
 #[test]fn omitted_rewritten_duplicate_and_unlinked_history_refuse(){
  let(b,a,o)=inputs();let mut variants=Vec::new();
  let mut bad=a.clone();bad["receipts"].as_array_mut().unwrap().pop();variants.push(bad);
  let mut bad=a.clone();bad["receipts"][0]["receipt_id"]=json!("rewritten");variants.push(bad);
  let mut bad=a.clone();bad["receipts"][1]["receipt_id"]=json!("r1");variants.push(bad);
  let mut bad=a.clone();bad["revisions"][0]["amount"]=json!(1);variants.push(bad);
  let mut bad=a.clone();bad["revisions"][1]["revision_id"]=json!("unlinked");variants.push(bad);
  let mut bad=a.clone();bad["head_revision_id"]=json!("v1");variants.push(bad);
  let mut bad=a.clone();bad["domain_effects"]=json!(["business-write"]);variants.push(bad);
  for bad in variants{assert!(validate_history_delta(&b,&bad,&o).is_err());}
 }
 #[test]fn new_receipt_cannot_point_to_old_head_while_new_revision_is_orphaned(){let(b,mut a,mut o)=inputs();a["receipts"][1]["revision_id"]=json!("v1");a["head_revision_id"]=json!("v1");o["acknowledgements"][0]["revision_id"]=json!("v1");assert!(validate_history_delta(&b,&a,&o).is_err());}
 #[test]fn unrelated_ack_and_stale_command_receipt_refuse(){let(b,a,o)=inputs();for key in ["command_id","receipt_id","revision_id"]{let mut wrong=o.clone();wrong["acknowledgements"][0][key]=json!("unrelated");assert!(validate_history_delta(&b,&a,&wrong).is_err());}let mut stale=o;stale["refused_command"]=json!("c2");assert!(validate_history_delta(&b,&a,&stale).is_err());}
 #[test]fn missing_or_duplicate_client_observation_refuses(){let(b,a,mut o)=inputs();o["acknowledgements"]=json!([]);assert!(validate_history_delta(&b,&a,&o).is_err());o["manual_commands"]=json!(["c2","c2"]);assert!(validate_history_delta(&b,&a,&o).is_err());}
}
