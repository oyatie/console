...
async fn run_case(pool:PgPool,project:&str,title:&str,kind:Kind)->TestResult{
 ...
 let baseline_wage_history=match &prepared.target{console_app::ui30::UiTarget::Draft{..}=>Some(serde_json::to_value(console_payroll_adapter_postgres::action30::read_contract_wage_history(&f.pool,&actor,f.facts.employee_id).await?)?),_=>None};
 ...
 let post=match &prepared.target{
  console_app::ui30::UiTarget::Draft{draft_id}=>{
   ...
   let wage_history=serde_json::to_value(console_payroll_adapter_postgres::action30::read_contract_wage_history(&f.pool,&actor,f.facts.employee_id).await?)?;
   assert_eq!(Some(&wage_history),baseline_wage_history.as_ref(),"editing/saving a draft must not change actual contract wage history");
   ...
   json!({"acknowledged":current,"history_manifest":history,"actual_wage_history":wage_history})
  },
...
async fn draft_history(pool:&PgPool,actor:&console_platform_request_context::account::AuthenticatedCompanyContext,id:uuid::Uuid)->TestResult<Value>{
 ...
 let mut pages=0;
 loop{pages+=1;assert!(pages<=manifest.page_budget,"history exceeds declared page budget before fetch");let page=owner::read_draft_history_page(pool,actor,&manifest,cursor.as_deref()).await?;assert_eq!(page.manifest,manifest);
