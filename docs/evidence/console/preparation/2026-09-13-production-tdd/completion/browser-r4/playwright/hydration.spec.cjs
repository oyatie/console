...
async function openHydrated(page,f){
 await page.addInitScript(observeWasm);
 ...
 await expect.poll(()=>page.evaluate(hash=>window.__consoleObservedWasmInstances().some(x=>x.sha256===hash),sha256)).toBe(true);
 return {url:response.url(),sha256,instances:await page.evaluate(()=>window.__consoleObservedWasmInstances())};
}
