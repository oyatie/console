...
use crate::{native_fixture::{fixture,Fixture,TestResult},binder_sequence::{bind_and_seal,completed,explicit_new_intent},commit_ack_relay::CommitAckRelay};
use sqlx::PgPool;
...
macro_rules! family_tests {
 ...
 async fn $revocation_test(pool:PgPool)->TestResult {
  ...
  crate::assert_registered_refusal!(&f.pool,&f.reviewer,&c.exact_registered_target,&$revoke(wrong),payroll::$revoke_owner);
  ...
  let a=bind_and_seal(&f.pool,&f.reviewer,&target,&$revoke(input)).await?;
  let relay=CommitAckRelay::start(&f.pool).await?;
 ...
 async fn $race_test(pool:PgPool)->TestResult {
  ...
  let one=bind_and_seal(&f.pool,&f.reviewer,&c.exact_registered_target,&input).await?;
  let target=explicit_new_intent::<$revoke>(&f.pool,&f.reviewer,&c.exact_registered_target).await?;
  let two=bind_and_seal(&f.pool,&f.reviewer,&target,&input).await?;
