...
#[sqlx::test(migrations = false)]
async fn ctx_thirty_three_real_group_candidates_refuse_whole_scan_instead_of_truncating(
    pool: PgPool,
) -> Result<()> {
    let f = ContextFixture::create(&pool, 33, 1).await?;
    let (status, body, _) = f
        .page(
            &f.accounts.submitter.account_access_token,
            "/api/v2/accounts/me/contexts?page_size=1",
    ...
    assert_eq!(body["error"]["code"], "navigation_unavailable");
    assert!(body.get("contexts").is_none() && body.get("next_cursor").is_none());
    Ok(())
}
#[sqlx::test(migrations = false)]
async fn ctx_two_hundred_fifty_seven_real_company_candidates_refuse_without_partial_disclosure(
    pool: PgPool,
) -> Result<()> {
    let f = ContextFixture::create(&pool, 1, 257).await?;
    let (status, body, _) = f
        .page(
            &f.accounts.submitter.account_access_token,
            "/api/v2/accounts/me/contexts?page_size=1",
