...
pub async fn observe(
    ...
    for (ordinal, event) in
        console_app::console_telemetry::schedule_projection_events(p, env)?
            .into_iter()
            .enumerate()
    {
        events.push(json!({"ordinal":ordinal,"bytes":console_app::console_telemetry::encode_safe_operational_event(&event)?}));
    }
