...
macro_rules! family_tests {
 ...
 async fn $verification_test(pool:PgPool)->TestResult {
  ...
  // Positive setup may advance Company/family control. Re-resolve the original
  // proposal's current control for BOTH actual actors after that setup.
  let current=payroll::$reader(&f.pool,&f.reviewer,&proposed.command,&proposed.effect_ordinals).await?;
  assert_eq!(current.state,payroll::AdmissionState::Proposed);
  assert_eq!(current.admission_ref,c.admission_ref);
  assert_eq!(current.head_revision,c.head_revision);
  assert_eq!(current.admission_digest,c.admission_digest);
  assert_eq!(current.current_reviewer_credential_ref,c.current_reviewer_credential_ref);
  let alias=payroll::$reader(&f.pool,&f.same_human_other_account,&proposed.command,&proposed.effect_ordinals).await?;
  assert_eq!(alias.state,payroll::AdmissionState::Proposed);
  assert_eq!(alias.admission_ref,current.admission_ref);
  assert_eq!(alias.head_revision,current.head_revision);
  assert_eq!(alias.admission_digest,current.admission_digest);
  assert!(alias.current_reviewer_credential_ref.is_some());
  let c=current;
 ...
 async fn $revocation_test(pool:PgPool)->TestResult {
  ...
  let expected_retained_input=serde_json::to_value(&$revoke(input.clone()))?;
  ...
  assert_eq!(retained_input,expected_retained_input,"entire source19 head/evidence input remains unchanged");
