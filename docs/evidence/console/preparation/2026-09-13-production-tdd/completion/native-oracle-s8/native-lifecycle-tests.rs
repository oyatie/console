...
async fn review_withdraw_keeps_immutable_decision_and_replays_control_once(pool:PgPool)->TestResult {
 ...
 let before_controls=governance::read_request_control_history(&f.pool,&f.reviewer,&request).await?;
 let before_accepted=serde_json::to_value(payroll::read_current_cycle_accepted_decisions(&f.pool,&f.submitter,&cycle.cycle).await?)?;
 // Same actual decision author and body, changing only explicit common reason.
 // Normal draft keeps both values; ordinary sealing/owner validation refuses
 // disagreement without withdrawing or silently overwriting body.reason.
 let conflicting=explicit_new_intent::<governance::ReviewWithdraw>(&f.pool,&f.reviewer,&target).await?;
 crate::assert_registered_refusal!(&f.pool,&f.reviewer,&conflicting,&input,governance::review_withdraw; reason = Some("Different common reason that must not overwrite the actual input"));
 assert_eq!(original,governance::read_review_decision_history(&f.pool,&f.reviewer,&request).await?);
 assert_eq!(before_controls,governance::read_request_control_history(&f.pool,&f.reviewer,&request).await?);
 assert_eq!(before_accepted,serde_json::to_value(payroll::read_current_cycle_accepted_decisions(&f.pool,&f.submitter,&cycle.cycle).await?)?);
 // Positive same-actor control uses a new explicit intent and matching values.
 let matching=explicit_new_intent::<governance::ReviewWithdraw>(&f.pool,&f.reviewer,&target).await?;
 let a=crate::binder_sequence::bind_and_seal_with_reason(&f.pool,&f.reviewer,&matching,&input,Some(input.reason.as_str())).await?;
 let relay=CommitAckRelay::start(&f.pool).await?;
