...
async fn mapping_partition(
    ...
    let targets:Vec<(Uuid,Uuid,Option<Uuid>)>=sqlx::query_as("SELECT m.source_id,m.target_account_id,a.id FROM account_migration_mappings m LEFT JOIN accounts a ON a.id=m.target_account_id WHERE m.manifest_sha256=decode($1,'hex') ORDER BY m.source_id,m.source_version")
        .bind(manifest).fetch_all(pool).await?;
    assert_eq!(targets.len(), mapped_count);
    for (source, target, actual) in targets {
        assert_eq!(source, target, "Account13 preserves old identity IDs");
        assert_eq!(actual, Some(target), "mapped target must actually exist");
    }
...
async fn m1_real_cutover_preserves_active_revoked_and_unresolved_legacy_material(
    ...
    let targets:Vec<Uuid>=sqlx::query_scalar("SELECT target_account_id FROM account_migration_mappings WHERE manifest_sha256=decode($1,'hex') AND source_id=$2")
        .bind(manifest.digest()).bind(unresolved).fetch_all(&pool).await?;
    assert!(
        targets.len() <= 1,
        "source must not split into fabricated Accounts"
    );
    if let Some(target) = targets.first() {
        // Account13 migration preserves source IDs; query the actual mapped ID
        // first so an erroneous new target cannot evade the Human assertion.
        assert_eq!(
            *target, unresolved,
            "Account mapping changed preserved identity"
        );
        let fabricated: i64 =
        ...
        assert_eq!(
            fabricated, 0,
            "unresolved source cannot manufacture Human authority"
        );
    } else {
        let quarantined:i64=sqlx::query_scalar("SELECT count(*) FROM account_migration_quarantine WHERE manifest_sha256=decode($1,'hex') AND source_id=$2 AND reason_code='UNRESOLVED_IDENTITY_PROVENANCE'")
            .bind(manifest.digest()).bind(unresolved).fetch_one(&pool).await?;
        assert_eq!(
            quarantined, 1,
            "unmapped unresolved source must retain explicit quarantine"
        );
    }
    Ok(())
}
...
async fn m5_more_than_500_sources_form_exact_restartable_partition(pool: PgPool) -> Result<()> {
    ...
    loop {
        ...
        if receipt.next_cursor.is_some() {
            assert!(
                !receipt.source_keys.is_empty(),
                "continued batch made no source progress"
            );
        }
        for key in &receipt.source_keys {
            assert!(expected.contains(key), "unexpected source/version");
...
async fn m5_byte_budget_is_independent_of_row_limit(pool: PgPool) -> Result<()> {
    ...
    let mut batches = 0;
    loop {
        assert!(cursors.insert(cursor.clone()), "byte-budget cursor cycle");
        ...
        .await?;
        assert!(receipt.source_bytes <= 4 * 1024 * 1024 && receipt.source_keys.len() < 500);
        assert!(
            !receipt.source_keys.is_empty(),
            "byte batch made no progress"
        );
        for key in &receipt.source_keys {
            assert!(expected.contains(key));
            assert!(
                seen.insert(key.clone()),
                "source consumed twice across byte batches"
            );
        }
        batches += 1;
        assert!(batches <= expected.len());
        match receipt.next_cursor {
    ...
    assert!(
        batches >= 2,
        "greater-than4MiB source requires at least two batches"
    );
    assert_eq!(seen, expected);
    let (mapped, quarantined) = mapping_partition(&pool, manifest.digest()).await?;
    assert_eq!(
