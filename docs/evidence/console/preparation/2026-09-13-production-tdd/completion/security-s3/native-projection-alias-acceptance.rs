...
async fn grant_visible_line(d: &NativeDeployment, run: uuid::Uuid) -> TestResult {
    ...
    .await?;
    let resource =
        payroll::read_native_line_policy_resource(&d.pool, &f.submitter, run, f.facts.employee_id)
            .await?;
...
async fn native_aliases_hide_other_company_and_other_employee_while_emitting_permitted_salary(
    pool: PgPool,
) -> TestResult {
    ...
    make_hidden_wage(&d, 0, "8424482").await?;
    let (after, _) = read_all(&d, &auth, run.run_id).await?;
    assert_eq!(
        after, before,
        "same-Company hidden employee influenced visible scope"
    );
