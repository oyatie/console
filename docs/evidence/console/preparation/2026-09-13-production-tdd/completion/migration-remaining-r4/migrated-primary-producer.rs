...
pub async fn activate_and_login(
    ...
    assert_eq!(protected(pool).await?, before);
    // Expiry is the actual ordinary ceremony timestamp. No shortened policy,
    // fake time, expiry UPDATE, or fabricated consumed status is introduced.
    let mut expired_nonce = [0u8; 32];
    rand::rngs::OsRng.fill_bytes(&mut expired_nonce);
    let expiring = accounts::start_account_primary_activation(
        &runtime.auth,
        &transport.service,
        accounts::PrimaryActivationStart {
            account_id: transport.key.account_id,
            terms: terms.reference.clone(),
            browser_nonce_digest: hex::encode(Sha256::digest(expired_nonce)),
        },
    )
    .await?;
    let (expires_at, issued_now): (time::OffsetDateTime, time::OffsetDateTime) = sqlx::query_as(
        "SELECT expires_at,clock_timestamp() FROM auth_webauthn_ceremonies WHERE id=$1",
    )
    .bind(expiring.ceremony_id)
    .fetch_one(pool)
    .await?;
    assert!(expires_at > issued_now);
    assert!(
        expires_at - issued_now <= time::Duration::minutes(5),
        "existing actual5minute ceremony policy"
    );
    let mut challenge = serde_json::to_value(&expiring.challenge)?;
    challenge["publicKey"]["allowCredentials"] = json!([{"type":"public-key","id":serde_json::from_str::<serde_json::Value>(&transport.key.credential_id)?}]);
    let expired_assertion = transport.key.authenticator.do_authentication(
        Url::parse(ORIGIN)?,
        serde_json::from_value::<RequestChallengeResponse>(challenge)?,
    )?;
    ...
    .await??;
    let before_expired = protected(pool).await?;
    let denied = accounts::finish_account_primary_activation(
        &runtime.auth,
        &transport.service,
        accounts::PrimaryActivationFinish {
            activation_id: expiring.activation_id,
            ceremony_id: expiring.ceremony_id,
            browser_nonce: hex::encode(expired_nonce),
            terms: terms.reference.clone(),
            credential: expired_assertion,
        },
    )
    .await;
    assert!(matches!(
    ...
    assert_eq!(protected(pool).await?, before_expired);
    let mut nonce = [0u8; 32];
    rand::rngs::OsRng.fill_bytes(&mut nonce);
    ...
    // Exact consumed successful proof is replayed without a fresh ceremony or
    // assertion. A second Account session/assent/security transition is forbidden.
    let before_replay = protected(pool).await?;
    let replay = accounts::finish_account_primary_activation(
        &runtime.auth,
        &transport.service,
        accounts::PrimaryActivationFinish {
            activation_id: started.activation_id,
    ...
    assert_eq!(protected(pool).await?, before_replay);
