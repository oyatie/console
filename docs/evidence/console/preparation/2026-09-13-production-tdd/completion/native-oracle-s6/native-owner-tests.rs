...
use crate::native_owner_producer::{complete_native,produce_native_and_review};
use crate::native_fixture::{fixture,TestResult};
use uuid::Uuid;
use sqlx::PgPool;
use crate::{binder_sequence,native_money_assertions};

#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_owner_chain_produces_complete_nonpayable_review(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_owner_chain_produces_complete_nonpayable_review").await?;
    let mut choices=f.facts.choices.clone();choices.create.correlation_id=Uuid::new_v4();
    ...
    native_money_assertions::assert_native_money(&durable);
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_first_prepare_is_durable_pending_before_terminal_effects(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_first_prepare_is_durable_pending_before_terminal_effects").await?;let (run,choices)=f.staged(false).await?;
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_prepare_replay_returns_same_receipt_without_second_input(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_prepare_replay_returns_same_receipt_without_second_input").await?;let (first,attempt)=f.prepared(false).await?;
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_calculation_replay_returns_same_money_and_receipt(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_calculation_replay_returns_same_money_and_receipt").await?;let (first,attempt)=f.calculated().await?;
    let before=f.snapshot(first.run_id).await?;
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_same_draft_control_command_changed_input_refuses(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_same_draft_control_command_changed_input_refuses").await?;let (run,choices)=f.staged(false).await?;
...
async fn refuse_changed_expectation(pool:PgPool,field:&str)->TestResult {
    let f=fixture(pool,&format!("expected_{field}")).await?;let (run,_)=f.closed().await?;
    ...
    let before=f.snapshot(run.run_id).await?;
    crate::assert_registered_refusal!(&f.pool,&f.submitter,&UntrustedActionTargetSelection::existing_run(run.run_id),&input,payroll::payroll_calculate_run);
    assert_eq!(before,f.snapshot(run.run_id).await?);
    // Same actor and owner accept the actual unchanged expected control.
    let selection=crate::binder_sequence::explicit_new_intent::<payroll::RunCalculate>(&f.pool,&f.submitter,&UntrustedActionTargetSelection::existing_run(run.run_id)).await?;
    let good_attempt=bind_and_seal(&f.pool,&f.submitter,&selection,&payroll::RunCalculate{expected:good}).await?;
    let initial=payroll::payroll_calculate_run(&f.pool,&f.submitter,&good_attempt).await?;
    let success=crate::native_owner_producer::complete_native(&f.pool,&f.submitter,&f.worker,&good_attempt,initial).await?;
    assert_eq!(success.state,payroll::RunState::Calculated);assert!(!success.payable);Ok(())
}
macro_rules! expectation_test {($name:ident,$field:literal)=>{#[sqlx::test(migrations="../crates/platform/db/migrations")] async fn $name(pool:PgPool)->TestResult{refuse_changed_expectation(pool,$field).await}};}
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_owner_wage_change_after_building_prevents_old_activation(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_owner_wage_change_after_building_prevents_old_activation").await?;let (run,choices)=f.staged(false).await?;
    let input=f.prepare_input(run.run_id,&choices).await?;
    let attempt=bind_and_seal(&f.pool,&f.submitter,&UntrustedActionTargetSelection::existing_run(run.run_id),&input).await?;
    let initial=payroll::payroll_prepare_inputs(&f.pool,&f.submitter,&attempt).await?;
    ...
    let correction_attempt=bind_and_seal(&f.pool,&f.submitter,&f.facts.wage_selection,&f.facts.wage_correction).await?;
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_blocked_population_cannot_close(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_blocked_population_cannot_close").await?;let (run,_)=f.prepared(true).await?;
    let before=f.snapshot(run.run_id).await?;
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn native_two_original_calculate_intents_have_one_current_winner(pool:PgPool)->TestResult {
    let f=fixture(pool,"native_two_original_calculate_intents_have_one_current_winner").await?;let (run,_)=f.closed().await?;
    ...
    let second_selection=binder_sequence::explicit_new_intent::<payroll::RunCalculate>(&f.pool,&f.submitter,&selection).await?;
    let two=bind_and_seal(&f.pool,&f.submitter,&second_selection,&payroll::RunCalculate{expected}).await?;
    assert_ne!(one.attempt_id,two.attempt_id);assert_ne!(one.command_id,two.command_id);
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn review_partial_approvals_do_not_produce_completion(pool:PgPool)->TestResult {
    let f=fixture(pool,"review_partial_approvals_do_not_produce_completion").await?;let review=f.review_open().await?;
    ...
    let submit=governance::ReviewSubmit{basis:basis.clone(),attest:true,submitted_evidence:f.facts.choices.review_evidence.clone()};
...
#[sqlx::test(migrations="../crates/platform/db/migrations")]
async fn review_second_account_of_same_human_cannot_self_approve(pool:PgPool)->TestResult {
    let f=fixture(pool,"review_second_account_of_same_human_cannot_self_approve").await?;let review=f.review_open().await?;
