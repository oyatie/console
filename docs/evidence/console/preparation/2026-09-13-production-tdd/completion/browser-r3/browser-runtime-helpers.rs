...
pub fn validate_history_delta(before:&Value,after:&Value,observation:&Value)->TestResult {
 ...
 if receipt["revision_id"]!=after["head_revision_id"]||!revisions.iter().any(|r|r["revision_id"]==receipt["revision_id"]){return Err("new receipt is not retained head revision".into());}
