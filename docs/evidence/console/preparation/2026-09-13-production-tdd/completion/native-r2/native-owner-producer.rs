...
async fn complete_native(
    ...
    initial: OwnerExecution<payroll::RunResult>,
) -> Result<payroll::RunResult, ProducerStop> {
    let mut pending = match initial {
        OwnerExecution::Completed(result) => return Ok(result),
...
pub async fn produce_native_and_review(
    ...
    let prepared = complete_native(pool, submitter, worker,
        payroll::payroll_prepare_inputs(pool, submitter, &prepare_attempt).await?).await?;
    if prepared.state != payroll::RunState::ReadyForReview || prepared.input.is_none() {
    ...
    let calculated = complete_native(pool, submitter, worker,
    ...
    for basis in bases {
        ...
        let decision_selection = UntrustedActionTargetSelection::existing_governance_request(request.request_id);
