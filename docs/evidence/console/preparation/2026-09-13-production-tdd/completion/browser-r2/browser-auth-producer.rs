...
pub struct Browser {
    ...
    pub received_set_cookie: Vec<(String, i64)>,
    pub proof: String,
...
impl Browser {
    ...
    pub fn absorb(&mut self, h: &reqwest::header::HeaderMap) -> TestResult {
        for value in h.get_all(reqwest::header::SET_COOKIE) {
            ...
            self.received_set_cookie.push((raw.to_owned(), i64::try_from(now)?));
            let pair = raw.split(';').next().ok_or("cookie")?;
            let (k, v) = pair.split_once('=').ok_or("cookie pair")?;
            let parsed = cookie::Cookie::parse(raw.to_owned())?;
            let deleted = if let Some(age) = parsed.max_age() { age.whole_seconds() <= 0 }
                else { parsed.expires_datetime().is_some_and(|expiry| expiry.unix_timestamp() <= i64::try_from(now).unwrap()) };
            if v.is_empty() || deleted {
