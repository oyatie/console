...
impl CustodyFixture {
    pub fn new(label: &str, database: &str) -> Result<Self> {
        ...
        let manifest =
            serde_json::to_vec(&json!({"format_version":1,"fixture_only":true,"items":[{
            "terms_kind":"test.account.service","title":"TEST_ONLY service","locale":"en-US",
            "content_sha256":content_digest,"required":true}]}))?;
        ...
        let config = json!({"format_version":1,"environment":"TEST_ONLY",
            ...
            "content_authorities":{"test.content":{"key_path":"content.pem",
                "qualified_terms_kinds":["test.account.service"]}},
            "registered_terms_kinds":["test.account.service"],
    ...
    pub fn approve_manifest(&self, expected: i64, manifest: &[u8]) -> Result<ApprovedPublication> {
        ...
        let qualification = serde_json::to_vec(&json!({"kind":"TEST_ONLY_CONTENT_QUALIFICATION",
            "manifest_sha256":manifest_digest,"terms_kinds":["test.account.service"],
