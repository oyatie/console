...
class HistoryOracleTests(unittest.TestCase):
    ...
    def test_rejects_omitted_ack_even_with_self_consistent_counts(self):
        h,a=fixture();h["revisions"].pop();h["declared_revision_count"]=1;h["head_revision_id"]="r1";h["receipts"].pop();h["declared_receipt_count"]=1
    ...
    def test_rejects_two_commands_claiming_same_receipt(self):
        h,a=fixture();h["receipts"][1]["receipt_id"]=h["receipts"][0]["receipt_id"]
        with self.assertRaisesRegex(InvalidHistory,"duplicate receipt identity"):validate(h,[])

    def test_rejects_receipt_to_missing_revision(self):
        h,a=fixture();h["receipts"][1]["revision_id"]="missing"
        with self.assertRaisesRegex(InvalidHistory,"missing revision"):validate(h,[])

    def test_append_rejects_missing_old_receipt(self):
        h,a=fixture();after=history(h["revisions"]+[row(3,{"note":"new","amount":3100000})])
        after["receipts"].pop(0);after["declared_receipt_count"]-=1
        with self.assertRaisesRegex(InvalidHistory,"receipt rewritten"):extension(h,after,[],1)

    def test_append_accepts_preserved_old_and_new_receipts(self):
        h,a=fixture();after=history(h["revisions"]+[row(3,{"note":"new","amount":3100000})]);extension(h,after,a,1)
