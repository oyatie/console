...
pub async fn produce_attendance(
    ...
    let target=explicit_new_intent::<attendance::CalendarPublish>(pool,auth,&UntrustedActionTargetSelection::company()).await?;
    ...
    let coverage_input=attendance::CoverageResolve {
        ...
        source_package_ref:Some(attendance::EventPackageRef { artifact_ref:event_artifact.source_artifact,
        ...
        coverage_days_ref:Some(attendance::CoverageDaysRef { artifact_ref:day_artifact.source_artifact,
    ...
    if !blocked {
        ...
        let new_target=explicit_new_intent::<attendance::CloseBasisCreate>(pool,auth,&UntrustedActionTargetSelection::company()).await?;
        let attempt=bind_and_seal(pool,auth,&new_target,&input).await?;
