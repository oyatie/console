...
async fn pub_registration_holds_share_through_commit_and_publisher_only_waits_on_head(
    ...
    for (receipt, revision, event) in accepted {
        ...
        let actual: (Uuid, String, Value, Value) =
            sqlx::query_as("SELECT account_id,kind,evidence_ref,payload->'evidence' FROM account_security_events WHERE id=$1")
                .bind(event).fetch_one(&pool).await?;
        let expected = json!({"kind":"ACCOUNT_TERMS_RELEASE","receipt_id":old.id,"revision":old.revision.to_string(),"manifest_sha256":old.manifest_sha256});
        assert_eq!(
            actual,
            (
                finished.account_id,
...
#[sqlx::test(migrations = false)]
async fn pub_publisher_wins_real_head_lock_then_registration_refuses_without_activation(
    pool: PgPool,
) -> Result<()> {
    use console_platform_auth::{PasskeyService, WebauthnSettings, account_enrollment as accounts};
    use sha2::{Digest, Sha256};
    ...
    let before_accounts:Value=sqlx::query_scalar("SELECT jsonb_build_object('security',(SELECT to_jsonb(s) FROM account_security s WHERE account_id=$1),'acceptances',(SELECT COALESCE(jsonb_agg(to_jsonb(a) ORDER BY terms_kind,terms_version),'[]') FROM account_terms_acceptances a WHERE account_id=$1))").bind(start.account_id).fetch_one(&pool).await?;
    let mut barrier = pool.begin().await?;
    let barrier_pid: i32 = sqlx::query_scalar("SELECT pg_backend_pid()")
        .fetch_one(&mut *barrier)
        .await?;
    sqlx::query("LOCK TABLE account_terms_release_receipts IN SHARE MODE")
        .execute(&mut *barrier)
        .await?;
    let next = f.approve(1)?;
    let operator = f.authenticate(publisher::Environment::TestOnly).await?;
    let publishing = publisher::publish_terms(
        ...
        publisher::OperatorApprovalRef {
            kind: publisher::ApprovalKind::OperatorReleaseApproval,
            approval_sha256: next.approval_digest,
        },
    );
    tokio::pin!(publishing);
    let publisher_pid = tokio::select! {r=&mut publishing=>{r?;panic!("publisher bypassed receipt relation barrier")},p=blocked_pid(&pool,barrier_pid)=>p?};
    let finish = accounts::finish_account_enrollment(
        &auth,
        &service,
        accounts::EnrollmentFinish {
    ...
    tokio::pin!(finish);
    let finish_pid = tokio::select! {r=&mut finish=>{r?;panic!("finish passed actual publisher head lock")},r=&mut publishing=>{r?;panic!("publisher escaped receipt barrier")},p=blocked_pid(&pool,publisher_pid)=>p?};
    let account_lock:i64=sqlx::query_scalar("SELECT count(*) FROM pg_locks WHERE pid=$1 AND relation='account_security'::regclass AND granted").bind(finish_pid).fetch_one(&pool).await?;
    assert!(account_lock > 0);
    barrier.commit().await?;
    let (published, finished) = tokio::join!(publishing, finish);
    assert_eq!(published?.revision, old.revision + 1);
    assert!(matches!(
        finished,
        Err(accounts::AccountEnrollmentError::TermsChanged)
    ));
