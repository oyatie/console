...
The hub-aware native GJC transport guard is the installable plugin at `scripts/cursor/gjc-transport-guard/`. It intercepts `bash`/`edit`/`write` tool calls and keys grammar by session cwd path prefix against one shared policy file:

- Console hub (`/Users/jasonlee/Developer/console`): allow `scripts/cursor/provision-lane-worktree.sh` and `git worktree add <hub>/.worktrees/<name> -b lane/<id>|admission/<id> origin/main`; deny sibling worktree paths outside the hub.
- Oyatie hub (`/Users/jasonlee/Developer/oyatie`): allow `git fetch origin dev` and `git worktree add <hub>/.worktrees/lane-<id> -b agent/<id> origin/dev`; deny other worktree-add shapes.
- Both hubs: deny `pkill`/`killall`/`kill` aimed at git; deny `rm` of `.git` lock files (`index.lock`, `HEAD.lock`, `gc.pid`, `.git/**/*.lock`); deny `gh pr update-branch`; deny `mm-role`, `claude -p`, `codex exec`; deny model slugs matching `-fast`; deny `git push --force` without `--force-with-lease`.

Install (dry-run only from a lane; integration owner performs a real install):

```sh
gjc plugin install scripts/cursor/gjc-transport-guard --user --dry-run
```

The packed-source identity is the sha256 in `scripts/cursor/gjc-transport-guard/BUNDLE-HASH`, generated by `node scripts/cursor/gjc-transport-guard/hash-bundle.mjs`. Session start re-hashes installed files against the registry; drift quarantines the plugin (`runtime_mismatch`) and the hook no longer runs. The self-report / probe runner fails closed when `examined=0`.
