...
| Collaboration/work communications | `/collaboration`, `/messenger`, `/mail`, `/support`, `/support/new` | Messenger, platform-operated work mail, support tickets |
...
- **Manufacturing execution / MES**: SAP Digital Manufacturing (<https://www.sap.com/products/scm/digital-manufacturing.html>), SAP Digital Manufacturing help (<https://help.sap.com/docs/sap-digital-manufacturing>), Siemens Opcenter Execution (<https://www.siemens.com/en-us/products/opcenter/execution/>), Rockwell Plex MES (<https://plex.rockwellautomation.com/en-us/products/manufacturing-execution-system.html>), Tulip Frontline Operations Platform (<https://tulip.co/platform/>), Microsoft Dynamics 365 production floor execution (<https://learn.microsoft.com/en-us/dynamics365/supply-chain/supply-chain-dev/production-floor-execution-styles>).
...
| EP-006 | `/mail` | Work mail and secure correspondence | Gmail, Outlook, Proton Mail, Teams email-to-channel | Platform-operated account readiness, inbox/thread detail, reply/forward, outbound attachments | P1 | Drafts/templates/labels/filters/search plus object-linked mail sends; domain/mailbox admin must remain platform-native, not SMTP/IMAP server config |
...
| EP-017 | Future manufacturing execution/MES surfaces | Manufacturing execution, shop-floor work, quality, traceability, OEE | SAP Digital Manufacturing, Siemens Opcenter Execution, Rockwell Plex MES, Tulip, Dynamics production floor execution | Production personas are recognized in story rubric, but no MES module exists yet | P2 | Add MES only after org/people/assets/inventory/ERP/workflow/policy foundations; see `docs/specs/mes.md` |
...
### EP-017 — manufacturing execution / MES

- **Pain point**: production companies in the group eventually need plant/line/work-center execution that is as mature as asset, HR, ERP, and workflow modules, without forcing all tenants into manufacturing vocabulary.
- **Missing capabilities**: production orders, dispatch lists, work centers, routings/operations, operator work instructions, material consumption, WIP, quality checks/nonconformance, genealogy/lot/serial traceability, downtime/OEE, rework/scrap, labor capture, and production-floor dashboards.
- **UX weaknesses**: no shop-floor operator/supervisor screens exist; the future UX must prioritize scan/touch execution, abnormal-condition escalation, low-latency status, and supervisor exception handling instead of explanatory text.
- **Policy/audit/security gaps**: e-signature/step-up for quality release, rework/scrap, override, batch/lot disposition, and production-record correction; retention and audit rules must match legal/customer/quality-system obligations.
- **Data/model gaps**: needs `Plant`, `WorkCenter`, `ProductionOrder`, `Routing`, `Operation`, `MaterialLot`, `Serial`, `WipMove`, `QualityInspection`, `Nonconformance`, `DowntimeEvent`, `ScrapReworkEvent`, and OEE/traceability facts as typed ontology/workflow objects.
- **Integration gaps**: MES must connect to ERP orders/materials, inventory, assets/EAM, quality, HR time/labor, procurement, mail/messenger, approvals, analytics, and imports/exports.
- **E2E/user-story gaps**: needs operator start/complete operation, supervisor respond to NC/downtime, planner release/sequence work, quality release hold, material traceability recall, and executive OEE drilldown stories.
...
4. **EP-017 MES foundation design**: production/order/work-center/material/quality/OEE/traceability schema and UI/UX benchmark spec; do not implement until org, assets/inventory, ERP, workflow, policy, and import foundations exist.
5. **EP-015 analytics semantic layer**: Power BI/SAP Analytics-style dashboards that drill to semantic models and source objects.
...
2. **EP-017 MES optimization**: schedule sequencing, bottleneck/root-cause analysis, quality-yield optimization, predictive downtime, and WIP/inventory balancing only after the mechanical MES event model is trusted.
3. Algorithmic baselines first; ML/RL/LLM only after data quality, observability, and approval/write-back controls exist.
