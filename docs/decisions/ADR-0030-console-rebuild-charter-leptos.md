...
**Two of the six conditions are open. The gate is closed, and that is the
correct state.** One of the two — the read model — is still a defect the
Critic's B6 raised in `docs/ideas/ecosystem-plan-review.md`, which returned
ITERATE with `implementation_ready: false`; B6's other defect, projected
dispatch, is closed above. Gating on the engine rather than on the verticals
deliberately starts the console against a substrate whose entity model is under
active revision; that risk is the reason the conditions are stated as
measurements rather than as a judgement.
