...
| Condition | Status at 2026-07-30 | Evidence |
|---|---|---|
...
| The shared contracts crate exists and `backend/openapi/openapi.yaml` is generated from it | **OPEN** | `utoipa` appears nowhere in `backend/Cargo.toml` or `backend/Cargo.lock`; the 35,935-line `backend/openapi/openapi.yaml` is hand-maintained and served verbatim by `include_str!` at `backend/app/src/lib.rs:214`, and its `info.title` at `:3` is still the pre-rename `Maintenance FSM Backend API` |
| Actions on a projected type do not require a hand-written Rust closure per action | **OPEN** | `ProjectedDispatchRegistry` is a `#[derive(Default)]` empty `HashMap` (`backend/crates/ontology/rest/src/lib.rs:169-173`), constructed empty by the REST state at `:115`, and an unknown target fails closed on `ActionError::NotWiredYet` at `:186-195` |
| A read model exists for aggregate queries | **OPEN** | zero occurrences of `MATERIALIZED VIEW` across all 205 migrations in `backend/crates/platform/db/migrations/` |
| A real dry run exists | **OPEN** | `preflight_action` (`backend/crates/ontology/rest/src/lib.rs:1238-1257`) returns `would_execute: gates.allow && criteria_ok` at `:1252` without calling `apply_edits`, which appears only on the execute path at `:1364`; an invalid edit set is therefore reported as executable |

**Four of the six conditions are open. The gate is closed, and that is the
correct state.** Two of the four — projected dispatch and the read model — are
the same defects the Critic's B6 raised in
`docs/ideas/ecosystem-plan-review.md`, which returned ITERATE with
`implementation_ready: false`. Gating on the engine rather than on the
verticals deliberately starts the console against a substrate whose entity
model is under active revision; that risk is the reason the conditions are
stated as measurements rather than as a judgement.
...
**Forbidden until every condition in §7 is measured green:** implementation.
No mounted shell, no route, no component, no navigation entry. ADR-0025 §4's
rule that a slice includes the backend work or waits behind a named backend
charter (`:146-147`) applies with the sequencing inverted: the backend is
built first by decision, not discovered as a blocker.

**The CI assertion that no console route source exists therefore stays.**
`scripts/console/route-inventory.test.mjs:36`, run at
`.github/workflows/ci.yml:137`, is the enforcement mechanism for
planning-only. It is not an obstacle to retire opportunistically; it may be
retired only in the change that opens the gate, with a justified commit.
