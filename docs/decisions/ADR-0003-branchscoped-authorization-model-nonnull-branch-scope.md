...
## Decision
`Branch`/`Region` are first-class day-1 schema concepts. Principals carry a `BranchScope` (kernel type): `All` only where a built-in `Feature` capability or a live database membership proof derives it — never from a role name — and an explicit branch set otherwise. Composition with `org_id`, the single legal composition point, and the migration path for existing SUPER_ADMIN/EXECUTIVE principals are governed by ADR-0028. Repositories filter by scope by default (default-deny); cross-branch access is an authorization test fixture (T0.6). P1 broadcasts, KPI rollups, wall-boards, and team channels are branch-scoped.
