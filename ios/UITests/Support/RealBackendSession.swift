...
enum RealBackendSession {
    ...
    enum SeedError: Error, LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .missingBaseURL:
                return "CONSOLE_UITEST_BASE_URL is required for every iOS UI-test launch."
            case .missingAccessToken:
                return "CONSOLE_UITEST_ACCESS_TOKEN is missing or empty; the workflow must mint and inject a real access token."
            case .missingRefreshToken:
                return "CONSOLE_UITEST_REFRESH_TOKEN is missing or empty; the workflow must mint and inject its matching real refresh token."
            }
        }
    }
    ...
    static func baseURL(
        ...
        guard let value = environment["CONSOLE_UITEST_BASE_URL"], value.isEmpty == false,
    ...
    static func tokens(
        ...
        let accessToken = environment["CONSOLE_UITEST_ACCESS_TOKEN"] ?? ""
        guard accessToken.isEmpty == false else {
            throw SeedError.missingAccessToken
        }
        let refreshToken = environment["CONSOLE_UITEST_REFRESH_TOKEN"] ?? ""
