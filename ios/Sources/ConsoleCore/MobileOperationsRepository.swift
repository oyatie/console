...
public struct MobileOperationsRepository: Sendable {
    ...
    private let clock: any ConsoleClock
    ...
    public init(
        ...
        clock: any ConsoleClock = SystemFieldClock()
    ) {
    ...
    public func votePoll(
        ...
        guard selectedOptionIDs.isEmpty == false else {
            throw ConsoleGatewayError.unexpectedResponse("poll vote requires at least one option")
        }
    ...
    public func approveWorkOrder(
        ...
        guard approval.canExecuteOnMobile else {
            throw ConsoleGatewayError.unexpectedResponse("approval source cannot execute on mobile")
        }
        guard let stepUp else {
    ...
    public static func stepUpBinding(
        ...
        if let replayAttempt, replayAttempt < 1 {
            throw ConsoleGatewayError.unexpectedResponse("replay attempt must be 1-based")
        }
        ...
        case .mailSend, .workflowStepUp, .deviceRegistration, .onDutyPing:
            throw ConsoleGatewayError.unexpectedResponse("unsupported mobile passkey step-up action: \(actionKind.rawValue)")
        }
    ...
    private func submitPasskeyProtectedAction(
        ...
        case .approvalDecision:
            guard let objectID = action.objectID else {
                throw ConsoleGatewayError.unexpectedResponse("missing_approval_object_id")
            }
            try await gateway.approveWorkOrder(
        ...
        case .pollVote:
            guard let objectID = action.objectID else {
                throw ConsoleGatewayError.unexpectedResponse("missing_poll_object_id")
            }
        ...
        case .mailSend, .workflowStepUp, .deviceRegistration, .onDutyPing:
            throw ConsoleGatewayError.unexpectedResponse("unsupported mobile passkey replay action: \(action.actionKind.rawValue)")
        }
    }
    ...
    private func replayNonPasskeyAction(_ action: MobileQueuedSensitiveAction) async throws {
        ...
        case .deviceRegistration:
            guard let deviceID = action.deviceID,
                ...
                throw ConsoleGatewayError.unexpectedResponse("missing_device_registration_payload")
            }
            _ = try await gateway.registerDevice(deviceID: deviceID, appVersion: appVersion, pushToken: pushToken)
        case .onDutyPing:
            guard let locationPing = action.locationPing else {
                throw ConsoleGatewayError.unexpectedResponse("missing_on_duty_payload")
            }
            try await gateway.recordLocationPing(locationPing)
        case .approvalDecision, .mailSend, .pollVote, .workflowStepUp:
            throw ConsoleGatewayError.unexpectedResponse("unsupported non-passkey replay action: \(action.actionKind.rawValue)")
        }
    }
    ...
    private func validate(
        ...
        guard envelope.binding == expectedBinding else {
            throw ConsoleGatewayError.unexpectedResponse("passkey step-up binding mismatch")
        }
        guard envelope.assertion.ceremonyId != "00000000-0000-0000-0000-000000000000" else {
            throw ConsoleGatewayError.unexpectedResponse("passkey step-up ceremony id is required")
        }
    }
}
...
public extension MobileQueuedSensitiveAction {
    func stepUpBinding(
        ...
        guard let objectID else {
            throw ConsoleGatewayError.unexpectedResponse("missing_step_up_object_id")
        }
