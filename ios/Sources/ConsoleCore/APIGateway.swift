...
public enum ConsoleGatewayError: Error, Sendable, CustomStringConvertible {
...
public struct GeneratedTokenRefreshGateway: TokenRefreshGateway {
    ...
    public func refresh(using refreshToken: String) async throws -> AuthTokens {
        ...
        case let .undocumented(statusCode, _):
            throw ConsoleGatewayError.apiResponse(operation: "refreshToken", statusCode: statusCode)
        case .tooManyRequests:
            throw ConsoleGatewayError.apiResponse(operation: "refreshToken", statusCode: 429)
        }
    }
}
...
public protocol ConsoleAPIGateway: WorkOrderGateway, MessengerGateway, MobileOperationsGateway, PasskeyStepUpGateway {
...
public struct GeneratedConsoleAPIGateway: ConsoleAPIGateway, PasskeyAuthGateway {
    private let client: any APIProtocol
    ...
    public func startWorkOrder(id: Components.Schemas.Uuid) async throws {
        ...
        case let .undocumented(statusCode, _):
            throw ConsoleGatewayError.apiResponse(operation: "startWorkOrder", statusCode: statusCode)
        }
    }
    ...
    public func submitReport(id: Components.Schemas.Uuid, draft: ReportDraft) async throws {
        ...
        case let .ok(response):
            _ = try response.body.json
        case let .undocumented(statusCode, _):
            throw ConsoleGatewayError.apiResponse(operation: "submitWorkOrderReport", statusCode: statusCode)
        }
    }
    ...
    public func presignEvidence(_ request: Components.Schemas.EvidencePresignRequest) async throws -> Components.Schemas.EvidencePresignResponse {
        ...
        case let .serviceUnavailable(response):
            ...
            throw ConsoleGatewayError.temporaryServerFailure(statusCode: 503, message: message)
        case let .undocumented(statusCode, _) where ConsoleGatewayError.isRetryableHTTPStatus(statusCode):
            throw ConsoleGatewayError.temporaryServerFailure(
                statusCode: statusCode,
                message: "evidence presign returned HTTP \(statusCode)"
            )
        default:
            throw ConsoleGatewayError.unexpectedResponse("evidence presign returned \(String(describing: output))")
        }
    }
    ...
    public func confirmEvidence(evidenceID: Components.Schemas.Uuid) async throws -> Components.Schemas.EvidenceConfirmResponse {
        ...
        default:
            throw ConsoleGatewayError.unexpectedResponse("evidence confirm returned \(String(describing: output))")
        }
    }
