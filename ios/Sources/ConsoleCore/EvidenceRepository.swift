...
public struct EvidenceRepository: Sendable {
    ...
    private func putEvidence(data: Data, ticket: Components.Schemas.EvidencePresignResponse) async throws {
        guard let url = URL(string: ticket.upload.url) else {
            throw ConsoleGatewayError.invalidUploadURL(ticket.upload.url)
        }
        ...
        if let httpResponse = response as? HTTPURLResponse,
           !(200..<300).contains(httpResponse.statusCode) {
            ...
            if ConsoleGatewayError.isRetryableHTTPStatus(httpResponse.statusCode) {
                throw ConsoleGatewayError.temporaryServerFailure(
                    statusCode: httpResponse.statusCode,
                    message: message
                )
            }
            throw ConsoleGatewayError.unexpectedResponse(message)
        }
    }
    ...
    private static func isRetryableUploadFailure(_ error: Error) -> Bool {
        if let gatewayError = error as? ConsoleGatewayError {
