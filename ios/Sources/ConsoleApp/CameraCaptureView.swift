    ...
    var body: some View {
        VStack(spacing: 16) {
            ProgressView("capturing")
                .accessibilityIdentifier(ConsoleAccessibilityID.cameraPermissionRequesting)
            Button("camera_cancel", action: onCancel)
                .accessibilityIdentifier(ConsoleAccessibilityID.cameraCancelButton)
        }
        .padding()
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("camera_permission_denied")
                ...
                .accessibilityIdentifier(ConsoleAccessibilityID.cameraPermissionDenied)
            Button("camera_open_settings") {
                if let url = URL(string: UIApplication.openSettingsURLString) {
                    UIApplication.shared.open(url)
                }
            }
            .accessibilityIdentifier(ConsoleAccessibilityID.cameraOpenSettingsButton)
            Button("camera_cancel", action: onCancel)
                .accessibilityIdentifier(ConsoleAccessibilityID.cameraCancelButton)
        }
        .padding()
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("camera_unavailable")
                .multilineTextAlignment(.center)
                .accessibilityIdentifier(ConsoleAccessibilityID.cameraUnavailable)
            Button("camera_cancel", action: onCancel)
                .accessibilityIdentifier(ConsoleAccessibilityID.cameraCancelButton)
        }
        .padding()
...
private struct CameraPreviewController: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: Context) -> UIViewController {
        ...
        cancel.accessibilityIdentifier = ConsoleAccessibilityID.cameraCancelButton
        ...
        shutter.accessibilityIdentifier = ConsoleAccessibilityID.cameraShutterButton
...
private final class CameraSessionRunner: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.console.app.camera-session")
