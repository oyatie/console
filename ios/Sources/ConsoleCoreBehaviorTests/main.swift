...
@main
struct ConsoleCoreBehaviorTests {
    static func main() async throws {
        ...
        print("ConsoleCoreBehaviorTests passed")
    }
    ...
    private static func passkeyBootstrapRequestsBypassBearerRefreshAndRetry() async throws {
        ...
        let gateway = GeneratedConsoleAPIGateway(
    ...
    private static func legacyRefreshDeletionFailurePreventsNetworkAndClearsProvider() async throws {
        ...
        try legacy.write(encoded, service: "console.field", account: "console.field.session")
    ...
    private static func logoutDeletionFailureDoesNotClaimSignedOut() async throws {
        ...
        try await store.save(tokens)
        try legacy.write(try JSONEncoder().encode(tokens), service: "console.field", account: "console.field.session")
        legacy.failDeletes = true
        let repository = PasskeyAuthRepository(
    ...
    private static func workOrderRepositoryQueuesOnlyTransportFailuresForStartAndReport() async throws {
        ...
        for statusCode in permanentStatusCodes {
            ...
            startGateway.startError = ConsoleGatewayError.apiResponse(
            ...
            reportGateway.reportError = ConsoleGatewayError.apiResponse(
        ...
        let permanentProtocolFailures: [(label: String, error: Error)] = [
            (
                "http-500-contract",
                ConsoleGatewayError.temporaryServerFailure(
                    statusCode: 500,
    ...
    private static func keychainSessionStoreMigratesLegacyGroupSessionForward() async throws {
        ...
        try legacy.write(try JSONEncoder().encode(legacyTokens), service: "console.field", account: "console.field.session")
...
private final class RecordingMobileOperationsGateway: MobileOperationsGateway, @unchecked Sendable {
    ...
    var approvalPage = ConsoleCoreBehaviorTests.emptyApprovalItemsPage()
    var mailFolders = [ConsoleCoreBehaviorTests.generatedMailFolder()]
    var mailThreads = [ConsoleCoreBehaviorTests.generatedMailThread()]
    var calendarEvents = [ConsoleCoreBehaviorTests.generatedCalendarEvent()]
    var polls = [ConsoleCoreBehaviorTests.generatedPoll(submitted: false, voteCount: 0)]
    ...
    func votePoll(
        ...
        let updated = ConsoleCoreBehaviorTests.generatedPoll(submitted: true, voteCount: Int64(selectedOptionIDs.count))
        polls = [updated]
        return updated
    }
    ...
    func registerDevice(
        ...
        return ConsoleCoreBehaviorTests.generatedDevice(pushToken: pushToken)
    }
...
private final class RecordingEvidenceMaintenanceGateway: ConsoleAPIGateway, @unchecked Sendable {
    var errorToThrow: Error?
    ...
    func getWorkOrderDetail(id: Components.Schemas.Uuid) async throws -> TechnicianWorkOrder {
        ...
        return try ConsoleCoreBehaviorTests.expectNotNil(detailWorkOrder)
    }
...
private final class RecordingMessengerGateway: MessengerGateway, @unchecked Sendable {
    ...
    func sendMessage(
        ...
        return try ConsoleCoreBehaviorTests.expectNotNil(nextSentMessage).toMessengerMessage()
    }
...
private struct FixedClock: ConsoleClock {
    let date: Date
...
private final class MutableClock: ConsoleClock, @unchecked Sendable {
    var date: Date
