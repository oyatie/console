...
@MainActor
final class ConsoleViewModelReportLifecycleTests: XCTestCase {
    ...
    func testRejectedReportPreservesFailureState() async throws {
        let gateway = ControllableWorkOrderGateway(reportError: ConsoleGatewayError.apiResponse(operation: "submit", statusCode: 422))
        let viewModel = makeViewModel(gateway: gateway)
        viewModel.selectedWorkOrder = workOrder
        viewModel.diagnosis = "Resolved"
        viewModel.actionTaken = "Replaced"
    ...
    private func makeViewModel(gateway: ControllableWorkOrderGateway) -> ConsoleViewModel {
        ...
        let auxiliaryGateway = GeneratedConsoleAPIGateway(
            serverURL: URL(string: "https://api.example.com")!,
            tokenProvider: tokenProvider,
            sessionStore: InMemorySessionTokenStore(),
            transport: SuccessfulRefreshTransport()
        )
        let live = ConsoleAppContainer.live()
        return ConsoleViewModel(container: ConsoleAppContainer(
