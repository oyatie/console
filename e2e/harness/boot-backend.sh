#!/usr/bin/env bash
# Boot the console-app API role for e2e and block until /readyz is green.
#
...
# origin MUST be the Vite origin, not the backend's. CONSOLE_COOKIE_SECURE=false lets
# the HttpOnly console_refresh cookie set over plain http. CONSOLE_EMAIL_STUB_MODE=e2e
...
run_source_app() {
  ...
  ( cd "${BACKEND_DIR}" && \
    ...
    SQLX_OFFLINE=true cargo run -q -p console-app >"${LOG_FILE}" 2>&1 ) &
}
...
# ES256 keypair -> exports CONSOLE_JWT_PRIVATE_KEY_PEM / CONSOLE_JWT_PUBLIC_KEY_PEM.
# shellcheck source=./gen-keys.sh
. "$(dirname "${BASH_SOURCE[0]}")/gen-keys.sh"

export CONSOLE_APP_ROLE=api
...
export DATABASE_URL="postgres://console_rt:${CONSOLE_RT_POSTGRES_PASSWORD}@${PG_HOST}:${PG_PORT}/${DB_NAME}"
export LEAVE_COMMAND_DATABASE_URL="postgres://console_leave_cmd:${CONSOLE_LEAVE_COMMAND_POSTGRES_PASSWORD}@${PG_HOST}:${PG_PORT}/${DB_NAME}"
export ONTOLOGY_COMMAND_DATABASE_URL="postgres://console_ontology_cmd:${CONSOLE_ONTOLOGY_COMMAND_POSTGRES_PASSWORD}@${PG_HOST}:${PG_PORT}/${DB_NAME}"
export PLATFORM_FORCE_COMMAND_DATABASE_URL="postgres://console_platform_force_cmd:${CONSOLE_PLATFORM_FORCE_COMMAND_POSTGRES_PASSWORD}@${PG_HOST}:${PG_PORT}/${DB_NAME}"
export CONSOLE_HTTP_ADDR="${HTTP_ADDR}"
export CONSOLE_WEBAUTHN_RP_ID="${E2E_RP_ID:-localhost}"
export CONSOLE_WEBAUTHN_RP_ORIGIN="${E2E_RP_ORIGIN:-http://localhost:5173}"
export CONSOLE_COOKIE_SECURE=false
export CONSOLE_EMAIL_STUB_MODE=e2e
export CONSOLE_COLDSTART_OTP="${E2E_COLDSTART_OTP:-e2e-coldstart-otp-000}"
export CONSOLE_DISPATCH_JOBS_ENABLED=false
export RUST_LOG="${RUST_LOG:-info}"

echo "boot-backend: starting console-app api on ${CONSOLE_HTTP_ADDR}" >&2
if [[ -n "${CONSOLE_APP_BIN}" && -x "${CONSOLE_APP_BIN}" ]]; then
  ...
  if [[ -n "${newer_migration}" ]]; then
    echo "boot-backend: ${CONSOLE_APP_BIN} is older than ${newer_migration#"${REPO_ROOT}/"}; using source app" >&2
    run_source_app
  else
    "${CONSOLE_APP_BIN}" >"${LOG_FILE}" 2>&1 &
  fi
else
  run_source_app
fi
...
READY_URL="http://${CONSOLE_HTTP_ADDR}/readyz"
