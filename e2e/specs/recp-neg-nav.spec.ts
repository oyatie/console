 ...
 * bounced back to the default authenticated landing (/overview).
 *
 ...
 * route is gated by RequireDailyPlanRoute and bounces to /overview.
 */
...
test("RECP-NEG direct visit to /daily-plan bounces to /overview", async ({
  page,
  loginAs,
}) => {
  await loginAs("RECEPTIONIST");
  ...
  // /daily-plan is gated by RequireDailyPlanRoute → redirects to /overview.
...
test("RECP-NEG direct visit to /kpi bounces to /overview", async ({
  page,
  loginAs,
}) => {
  await loginAs("RECEPTIONIST");
