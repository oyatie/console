 ...
 *       visit by a non-admin is bounced to `/dispatch` and the CatalogAdminPage
 *       never renders.
 *
...
for (const role of NON_ADMIN_ROLES) {
  ...
  test(`NEG-NAV-CATALOG direct visit to /catalog bounces ${role} to /dispatch`, async ({
    page,
    loginAs,
  }) => {
    await loginAs(role);
    ...
    // /catalog is gated by RequireAdminRoute → redirect to /dispatch; the
