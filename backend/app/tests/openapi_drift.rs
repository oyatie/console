...
use mnt_app::objects::OBJECT_ROUTE_PATHS;
...
async fn openapi_yaml_covers_mounted_auth_routes() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for path in MESSENGER_ROUTE_PATHS {
        assert!(yaml.contains(path), "OpenAPI YAML is missing {path}");
    }
    ...
    for path in SUPPORT_ROUTE_PATHS {
        assert!(yaml.contains(path), "OpenAPI YAML is missing {path}");
    }
    for path in IDENTITY_ROUTE_PATHS {
        assert!(yaml.contains(path), "OpenAPI YAML is missing {path}");
    }
    ...
    for path in mnt_leave_rest::LEAVE_ROUTE_PATHS {
        assert!(yaml.contains(path), "OpenAPI YAML is missing {path}");
    }
    assert!(
