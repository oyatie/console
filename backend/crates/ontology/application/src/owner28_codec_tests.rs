...
use crate::owner28::{NormalizedOwnerSubmission, decode_owner_submission, encode_owner_submission};
...
fn rust_owner_direct_wage_matches_independent_literal_bytes() {
    ...
    assert_eq!(
        digest(expected),
        "8d27899205f60348204015c80d3dfa2c09db8caca3241426abc6c2ce0665170b"
    );
    let fixture: NormalizedOwnerSubmission =
        serde_json::from_slice(include_bytes!("codec-goldens/owner-direct-wage.json")).unwrap();
    let mut actual = Vec::new();
    let summary = encode_owner_submission(&fixture, &mut actual).unwrap();
    assert_eq!(actual.as_slice(), expected);
    assert_eq!(
        summary.command_fingerprint,
        "27a9fe6ee36df58348552d381dfeadfef06b06c551fe41b062aedc7cdb7fcc09"
    );
    assert_eq!(
        decode_owner_submission(&mut Cursor::new(expected))
            .unwrap()
            .command,
        fixture
    );
...
fn rust_owner_attempt_create_run_matches_independent_literal_bytes() {
    ...
    assert_eq!(
        digest(expected),
        "e962a57c7ed5f5e397fd607390e58561d3b40373e5bb091ab6298f2c967383df"
    );
    let fixture: NormalizedOwnerSubmission = serde_json::from_slice(include_bytes!(
        "codec-goldens/owner-attempt-create-run.json"
    ))
    .unwrap();
    let mut actual = Vec::new();
    let summary = encode_owner_submission(&fixture, &mut actual).unwrap();
    ...
    assert_eq!(
        summary.command_fingerprint,
        "b2ebcacabd4e8e0c289d9738c7e83ebf74faf10f534c3545111853fe29d51011"
    );
    assert_eq!(
        decode_owner_submission(&mut Cursor::new(expected))
            .unwrap()
...
fn rust_owner_gated_calculate_matches_independent_literal_bytes() {
    ...
    assert_eq!(
        digest(expected),
        "a08be2ac3ff8f5ad657bbe4a5fb8494956269af5e35949af4ff3ff7e721afcf0"
    );
    let fixture: NormalizedOwnerSubmission =
        serde_json::from_slice(include_bytes!("codec-goldens/owner-gated-calculate.json")).unwrap();
    let mut actual = Vec::new();
    let summary = encode_owner_submission(&fixture, &mut actual).unwrap();
    assert_eq!(actual.as_slice(), expected);
    assert_eq!(
        summary.command_fingerprint,
        "55ac174164f9e6e4a14b1e482db54f56ef1065eba4307953ee1c46da40fe1a8a"
    );
    assert_eq!(
        decode_owner_submission(&mut Cursor::new(expected))
            .unwrap()
...
fn rust_owner_publication_matches_independent_literal_bytes() {
    ...
    assert_eq!(
        digest(expected),
        "9489ebfea27dfc175711bb65425dff04628d7a2fdff2296a0084b77aa81ef6ad"
    );
    let fixture: NormalizedOwnerSubmission =
        serde_json::from_slice(include_bytes!("codec-goldens/owner-publication.json")).unwrap();
    let mut actual = Vec::new();
    let summary = encode_owner_submission(&fixture, &mut actual).unwrap();
    assert_eq!(actual.as_slice(), expected);
    assert_eq!(
        summary.command_fingerprint,
        "f57c46cc27e9ca2e80469729194fcade41d4de7a04ea47699ef72cef0555eaa7"
    );
    assert_eq!(
        decode_owner_submission(&mut Cursor::new(expected))
            .unwrap()
...
fn rust_owner_case_response_matches_independent_literal_bytes() {
    ...
    assert_eq!(
        digest(expected),
        "df2fcb0667d45d82075f504c5d36b07cb2be7639dcbf45142fc81e05a44326d5"
    );
    let fixture: NormalizedOwnerSubmission =
        serde_json::from_slice(include_bytes!("codec-goldens/owner-case-response.json")).unwrap();
    let mut actual = Vec::new();
    let summary = encode_owner_submission(&fixture, &mut actual).unwrap();
    assert_eq!(actual.as_slice(), expected);
    assert_eq!(
        summary.command_fingerprint,
        "9b17c48e47e0ebfb6b06764815ef9badf713bf2e34a9b4b6ff17e8db0c1944ac"
    );
    assert_eq!(
        decode_owner_submission(&mut Cursor::new(expected))
            .unwrap()
