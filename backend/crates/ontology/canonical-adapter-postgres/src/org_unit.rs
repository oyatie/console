...
///
/// Writes stay in THIS owner crate so `console-gate-writer-ownership` stays green
/// when org-change apply later calls the seam (Cargo.toml dep is a shared-root
/// follow-up — see lane receipt).
pub async fn ensure_unambiguous_legacy_binding_in_tx(
    ...
    if source_kind != SOURCE_KIND_REGION && source_kind != SOURCE_KIND_BRANCH {
        return Err(OrgUnitError::Blocked(vec![format!(
    ...
    if !attributes.is_object() {
        return Err(OrgUnitError::Blocked(vec![
