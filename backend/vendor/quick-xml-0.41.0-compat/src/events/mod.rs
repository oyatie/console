...
use crate::escape::{
    ...
    partial_escape, resolve_predefined_entity, unescape_with, EscapeError,
};
...
impl<'a> BytesText<'a> {
    ...
    ///
    /// Compatibility shim for crates that still call the quick-xml <=0.37 API.
    /// Prefer [`decode`](Self::decode), [`xml_content`](Self::xml_content), or
    /// `quick_xml::escape::unescape` in new code.
    ...
    pub fn unescape_with<'entity>(
        ...
        match unescape_with(&decoded, resolve_entity)? {
            // Because result is borrowed, no replacements were done and we can
            // return the decoded text directly.
