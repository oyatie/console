...
CARGO_PKG_VERSION = re.compile(r"CARGO_PKG_VERSION(?![0-9A-Z_])")


def resolved_package_version(version):
    """The version Cargo would compile with.

    A member that omits `version` from `[package]` still gets one -- Cargo
    documents the default as 0.0.0, and 23 members here rely on it. Refusing
    those would abort generation for the whole repository the first time any of
    them so much as mentioned the variable in a comment.

    Workspace inheritance is the one case that genuinely cannot be resolved
    here: `version.workspace = true` parses as a dict, and rendering that into
    BUCK would emit a Python repr as the crate version. `[workspace.package]`
    declares no version today, so this cannot fire; it raises rather than
    silently shipping garbage if that changes.
    """
    if version is None or version == "":
        return "0.0.0"
    if not isinstance(version, str):
        raise ValueError(
            "package version {!r} is inherited or non-scalar and cannot be "
            "resolved to a literal for Buck".format(version)
        )
    return version


def cargo_pkg_version_env(contents, version):
    """Cargo defines CARGO_PKG_VERSION for every crate; Buck2 does not, so a
    target that reaches for it through `env!` cannot compile under Buck at all.

    Scans the TARGET's own sources, like `cargo_bin_exe_env`, not the crate's.
    A crate-level scan attaches the variable to whichever targets happen to be
    generated from `src/`, which is both too broad (a library that never names
    it) and too narrow (the binary built from `main.rs`, and every integration
    test, are separate compilation units the crate scan never reaches).

    Matched by regex, not substring: `CARGO_PKG_VERSION_MAJOR` is a different
    variable, and defining VERSION for it would leave the build failing while
    the generator reported success.
    """
    if not CARGO_PKG_VERSION.search(contents):
        return {}
    return {"CARGO_PKG_VERSION": resolved_package_version(version)}


def file_text(path):
    """Whole-file text, empty when unreadable."""
    try:
...
def read_rs_sources(root, exclude=()):
    """Concatenated text of the `.rs` files under `root`, for target-scoped
    scans. `exclude` holds root-relative paths (a binary's `main.rs` is not part
    of its sibling library's compilation unit)."""
    blobs = []
    for dp, _, files in os.walk(root):
        for f in sorted(files):
            ...
            if os.path.relpath(path, root) in exclude:
                continue
            try:
                with open(path, encoding="utf-8", errors="ignore") as handle:
...
def emit(d, name, deps, named, dev_deps, dev_named, version=None):
    ...
    # never name it and miss the ones that do.
    lib_only = ("main.rs",) if (has_main and has_lib) else ()
